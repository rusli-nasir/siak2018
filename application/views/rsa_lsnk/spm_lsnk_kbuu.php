...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                ...
                if(confirm('Apakah anda yakin ?')){
                    ...
                    var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_lsnk_spm['nom'])?$detail_lsnk_spm['nom']:''?>' + '&deskripsi=' + 'LS-NON-KONTRAK <?=$alias?>' + '&nominal=' + jumlah_bayar + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&jumlah_pengembalian=' + '<?=$detail_lsnk['pengembalian']?>' + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&id_nomor_lsnk=' + '<?=$id_nomor_lsnk?>'  + '&tgl_proses=' + tgl_proses ;
                    $.ajax({
                        type:"POST",
                        url :"<?=site_url('rsa_lsnk/proses_final_lsnk')?>",
                        data:data,
        ...
        //                        $('#myModalKuitansi').modal('show');
                                if(data=='sukses'){
                                    window.location = '<?=site_url('rsa_lsnk/daftar_spm_kbuu/').$cur_tahun.'/'.$kd_unit?>';
                                }
        //                        
                        }
                    });
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_lsnk=' + '<?=$id_nomor_lsnk?>' + '&tgl_proses=' + '<?=date("Y-m-d")?>';
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_lsnk/proses_spm_lsnk')?>",
                data:data,
                success:function(data){
//                        console.log(data)
//                        $('#no_bukti').html(data);
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_lsnk == 'SPM-FINAL-VERIFIKATOR'){ ?>
