<script>
	...
	$(document).ready(function(){
		...
		function updateSum() {
			  var total = 0;
			  $(".ck:checked").each(function(i, n) {total += parseInt($(n).attr("data-harga"));})
	...
	$(".btn-spj_pumk").click(function(event){
		 event.preventDefault();
		var harga = updateSum1();
			...
			$.ajax({
				...
				success:function(respon){
					if(respon == "true"){
						...
						bootbox.alert({
									title: "SPJ",
									message: "SPJ MELEBIHI UANG PANJAR",
									// callback: function(){ 
...
<div id="page-wrapper" >
	<div id="page-inner">
		<div class="row">
			<div class="col-lg-12" >
				<div>
					<h2>DAFTAR KUITANSI</h2><hr>
				</div>
				<?php if ($this->check_session->get_level() == 13): ?>
					<div class="alert alert-danger" style="font-size: 18px">Uang Panjar Personal : <span style="color: red"><b>Rp.&nbsp;<?php echo number_format($dana,0,',','.') ?>,00</b> </span></div>
				...
				<table class="table table-bordered table-striped small"  id="o-table" >
					<thead>
						<tr>
							<td align="right" colspan="9">
								<div class="btn-group alert alert-success " id="buat_spj2">
									<b> Jumlah Kuitansi yang Aktif = <span id="total_harga" style="color: red;"></span></b> </br>
									<button type="button" class="btn-spj_pumk btn btn-primary btn-sm" id="spj_pumk" aria-label="Left Align" style="margin-right: 5px;">BUAT SPJ</button>
								</div>
							</td>
						</tr>																
						...
						<tr>
							<td align="right" colspan="9">
								<div class="btn-group alert alert-success " id="buat_spj" style="position: fixed; top: 80px; right: 50px; z-index: 1;display: none;								">
									<b>Jumlah Kuitansi yang Aktif = <span id="total_harga2" style="color: red;"></span> </b></br>
									<button type="button" class="btn-spj_pumk  btn btn-primary btn-sm" id="spj_pumk1" aria-label="Left Align" style="margin-right: 5px;">BUAT SPJ</button>
					...
					<tbody>
						<?php $total_kuitansi = 0; ?>
						<?php foreach ($daftar_spj_pumk as $key => $value): ?>
							<tr>
								...
								<td class="text-center">
									<div class="input-group">
										<span class="input-group-addon">
											...
											<?php else: ?>
												<input type="checkbox" aria-label="" data-harga="<?php echo $value->pengeluaran ?>" rel="<?=$value->id_kuitansi?>" class="ck" id="ck_<?=$value->id_kuitansi?>">
											<?php endif ?>
										</span>
									</div>
								</td>
						...
						<?php  $total_kuitansi +=  $value->pengeluaran;?>
					<?php endforeach ?>
					<tbody>
						<tr>
							<td colspan="4">&nbsp;	
							...
							<td>Jumlah</td>
							<td class="text-right"><?php echo number_format( $total_kuitansi) ?></td>
							<td colspan="3">&nbsp;</td>
						</tr>
					</tbody>
				</table>
