...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                ...
                if(confirm('Apakah anda yakin ?')){
                    ...
                    var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_up['nom'])?$detail_up['nom']:''?>' + '&deskripsi=' + 'LS3 <?=$alias?>' + '&nominal=' + jumlah_bayar + '&kuitansi_id=' + $('#kuitansi_id').html();
                    $.ajax({
                        type:"POST",
                        url :"<?=site_url('rsa_lsphk3/proses_final_lsphk3')?>",
                        data:data,
    ...
    $(document).on("click",'#proses_spm_kpa',function(){
        if(confirm('Apakah anda yakin ?')){
			var data = 'proses=' +'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM'  + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>'+ '&kuitansi_id=' + $('#kuitansi_id').html();
            $.ajax({
                type:"POST",
                            ...
                            <li>Kebenaran perhitungan dan isi tertuang dalam SPP ini menjadi tanggung jawab Bendahara Pengeluaran sepanjang sesuai dengan bukti-bukti pengeluaran yang telah ditandatangani oleh PPPK atau PPK</li>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2" style="vertical-align: top;line-height: 16px;padding-left: 10px;">
                                         <?php if(isset($detail_lsphk3['nom'])){ ?>
                                            <?php if($detail_lsphk3['nom'] >= 100000000){ ?>
                                            Setuju dibayar : <br>
...
<form action="<?=site_url('rsa_lsphk3/cetak_spm')?>" id="form_spm" method="post" style="display: none"  >
    <input type="text" name="dtable_2" id="dtable_2" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_lsphk3 =='SPM-FINAL-VERIFIKATOR'){ 
				//
				?>
