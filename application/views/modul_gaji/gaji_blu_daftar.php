...
$(function(){
	...
	$('.uang_makan').on('click', function(e){
		window.location = '<?php echo site_url('gaji_blu/daftar'); ?>';
	});
	$('.kesejaterahan').on('click', function(e){
		window.location = '<?php echo site_url('gaji_blu/daftar_2'); ?>';
	});
	$('.pajak').on('click', function(e){
		window.location = '<?php echo site_url('gaji_blu/daftar_3'); ?>';
	});
	$('.cetak_daftar').on('click', function(e){
		window.location = '<?php echo site_url('gaji_blu/daftar_cetak'); ?>';
	});
	
	$('#form-gajiblu-lihat').on('submit',function(e){
		e.preventDefault();
		$('#kriteria_gajiblu_lihat').modal('hide');
		$.post('<?php echo site_url('ikw/ikw_proses'); ?>',$('#form-gajiblu-lihat').serialize(),function(data){
			if(data!=1){
	...
	$('#sword_art_online').on('submit',function(e){
		e.preventDefault();
		$.post('<?php echo site_url('gaji_blu/gajiblu_proses'); ?>',$('#sword_art_online').serialize(),function(data){
			if(data!=1){
				$('#myModalMsg .body-msg-text').html(data);
				$('#myModalMsg').modal('show');
				return false;
	...
	$('#simpan_data').on('click',function(e){
		var a=confirm('Yakin menyimpan data ini?');
		if(a){
	...
	$('.trash').on('click',function(){
		...
		if(a){
			var id = $(this).attr('id');
			$.post('<?php echo site_url('gaji_blu/gajiblu_proses'); ?>',{'id':id, 'act':'gajiblu_hapus_single'},function(data){ 
				if(data!='1'){
		...
		<div class="row">
			<div class="col-md-12">
				<h4 class="page-header">
					<i class="fa fa-coffee"></i>&nbsp;&nbsp;Data Gaji Tenaga Kependidikan Non PNS (BLU) - <a href="<?php echo site_url('modul_gaji'); ?>" title="ke Menu Utama Kepegawaian">Kepegawaian #2</a>
				</h4>
				<p class="text-right">
        	<button type="button" class="btn btn-primary btn-sm home"><i class="fa fa-home"></i>&nbsp;&nbsp;&nbsp;Beranda Gaji Tenaga Kependidikan Non PNS (BLU)</button>
					<button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#kriteria_gajiblu_lihat"><i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;&nbsp;Lihat Daftar Gaji Tenaga Kependidikan Non PNS (BLU)</button>
				</p>
			</div>
		  <div class="col-md-12">
        <div class="col-md-12" style="background-color:#ccc;padding:5px;">
        	...
        	<div class="col-md-12" style="background-color:#fff;padding:3px;">
          	<h4 class="text-center">
            	Daftar Penerima Gaji Tenaga Kependidikan Non PNS (BLU) <?php echo $this->cantik_model->getJenisPeg($_SESSION['gaji_blu']['jnspeg']); ?><br/>Universitas Diponegoro<br/>
              Semester <?php echo $this->cantik_model->wordMonth($_SESSION['gaji_blu']['bulan']); ?> Tahun <?php echo $_SESSION['gaji_blu']['tahun']; ?>
            </h4>
            ...
            <div>
            	<ul class="nav nav-tabs" role="tablist" id="kentut_tab">
            		<li role="presentation" class="active"><a href="javascript:void();" role="tab" data-toggle="tab" aria-expanded="true" data-original-title="" title="Daftar Uang Makan" class="uang_makan"><i class="glyphicon glyphicon-list"></i>Uang Makan</a></li>
              	<li role="presentation"><a href="javascript:void();" role="tab" data-toggle="tab" aria-expanded="true" data-original-title="" title="Daftar Kesejaterahan" class="kesejaterahan"><i class="glyphicon glyphicon-list"></i>Daftar Tunj. Kesejaterahan</a></li>
                <li role="presentation"><a href="javascript:void();" role="tab" data-toggle="tab" aria-expanded="true" data-original-title="" title="Pajak" class="pajak"><i class="glyphicon glyphicon-list"></i>Daftar Pajak</a></li>
              </ul>
            </div>
            <form id="sword_art_online">
            <input type="hidden" name="act" value="gajiblu_simpan_uangmakan" />
...
<div class="modal fade" id="kriteria_gajiblu_lihat" tabindex="-1" role="dialog" arialabelledby="myModalLabel">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form-gajiblu-lihat" action="<?php echo site_url('ikw/ikw_proses'); ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-header">
          ...
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Cari Data Gaji Tenaga Kependidikan Non PNS (BLU)</h5>
        </div>
                ...
                <select name="jnspeg" id="jnspeg" class="form-control input-sm">
                  <?php
                    ...
                    foreach ($_jenispeg as $k => $v) {
                      $_s = "";
                      if(isset($_SESSION['gaji_blu']['jnspeg']) && $_SESSION['gaji_blu']['jnspeg']==$v[0]){
                        $_s = " selected";
                      }
                  ?>
