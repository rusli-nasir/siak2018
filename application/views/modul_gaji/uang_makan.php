<script type="text/javascript">
	$(function(){
		// untuk form
		$('#form-uang-makan').on('submit',function(e){
			e.preventDefault();
			$('#kriteria_uang_makan').modal('hide');
			$.post($(this).attr('action'),$(this).serialize(),function(data){
				if(data!='1'){
					...
					$('.result_data').load('<?php echo site_url('uangmakan/showDialogProsesUangMakan'); ?>');
					return false;
				}
			});
		});
		
		$('#form-uang-makan-lihat').on('submit',function(e){
			e.preventDefault();
			$('#kriteria_uang_makan_lihat').modal('hide');
			$.post('<?php echo site_url('uangmakan/uangmakan_proses'); ?>',$('#form-uang-makan-lihat').serialize(),function(data){
				if(data!=1){
					$('#myModalMsg .body-msg-text').html(data);
					$('#myModalMsg').modal('show');
					return false;
				}else{
					window.location='<?php echo site_url('uangmakan/daftar'); ?>';
					return false;
					//$('#form-ipp')[0].reset();
				}
			});
		...
		$('#kriteria_uang_makan').on('hidden.bs.modal', function () {
				$('#form-uang-makan')[0].reset();
				cek_checked();
		});
	...
	$(document).on('click','#proses_2',function(e){
		e.preventDefault();
		$.post('<?php echo site_url('uangmakan/uangmakan_proses'); ?>', {'act':'uangmakan_proses2'}, function(data){
			if(data==1){
				window.location='<?php echo site_url('uangmakan/daftar'); ?>';
			}else{
				$('#myModalMsg .body-msg-text').html(data);
		...
		<div class="row">
			<div class="col-md-12">
				<h4 class="page-header">
					<i class="fa fa-cutlery"></i>&nbsp;&nbsp;Data Uang Makan - <a href="<?php echo site_url('modul_gaji'); ?>" title="ke Menu Utama Kepegawaian">Kepegawaian #2</a>
				</h4>
				<p class="text-right">
					<button type="button" class="btn btn-default btn-flat btn-sm" data-toggle="modal" data-target="#kriteria_uang_makan_lihat"><i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;&nbsp;Lihat Daftar Uang Makan </button>
					<button type="button" class="btn btn-primary btn-flat btn-sm" data-toggle="modal" data-target="#kriteria_uang_makan"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Proses Uang Makan </button>
				</p>
			</div>
...
<div class="modal fade" id="kriteria_uang_makan" tabindex="-1" role="dialog" arialabelledby="myModalLabel">
  <!-- /.modal-content -->
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form-uang-makan" action="<?php echo site_url('uangmakan/uangmakan_proses'); ?>" method="post" enctype="multipart/form-data">
        <input type="hidden" id="act" name="act" value="uangmakan_proses"/>
        <input type="hidden" id="tahun" name="tahun" value="<?php echo $cur_tahun; ?>"/>
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span></button>
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Kriteria Proses Uang Makan </h5>
        </div>
...
<!-- /.modal-dialog untuk menambahkan item transaksi-->
<div class="modal fade" id="kriteria_uang_makan_lihat" tabindex="-1" role="dialog" arialabelledby="myModalLabel">
  <!-- /.modal-content -->
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form-uang-makan-lihat" action="<?php echo site_url('uangmakan/uangmakan_proses'); ?>" method="post" enctype="multipart/form-data">
        <input type="hidden" id="act" name="act" value="ikw_lihat"/>
        <input type="hidden" id="tahun" name="tahun" value="<?php echo $cur_tahun; ?>"/>
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span></button>
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Cari Data Uang Makan </h5>
        </div>
        <div class="modal-body">
          <span class="message"></span>
          <div class="row">
          ...
          <div class="row">
		      ...
		      <div class="row">
	            ...
	            <?php
								...
								foreach ($stt as $k => $v) {
									$ch = "";
									if(isset($_SESSION['uangmakan']['status']) && in_array($k,$_SESSION['uangmakan']['status'])){
										$ch = " checked = \"checked\"";
									}
							?>
		              <div class="small col-md-3 col-sm-6 col-xs-12">
          	...
          	<div class="col-md-4">
                ...
                <select name="jnspeg" id="jnspeg" class="form-control input-sm">
                  <?php
                    ...
                    foreach ($_jenispeg as $k => $v) {
                      $_s = "";
                      if(isset($_SESSION['uangmakan']['jnspeg']) && $_SESSION['uangmakan']['jnspeg']==$v[0]){
                        $_s = " selected";
                      }
                  ?>
                  <option value="<?php echo $v[0]; ?>"<?php echo $_s; ?>><?php echo $v[1]; ?></option>
