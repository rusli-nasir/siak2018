...
$(document).ready(function(){
    ...
    $(document).on("click",'#proses_spm_verifikator',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' +'SPM-FINAL-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM'  + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_tambah_ks=' + '<?=$id_nomor_tambah_ks?>';
            $.ajax({
                type:"POST",
...
//                        $('#myModalKuitansi').modal('show');
                        if(data=='sukses'){
                            // location.reload();
                            window.location = '<?=site_url('rsa_tambah_ks/daftar_spm_verifikator/').$cur_tahun.'/'.$kd_unit?>';
                        }
//                        
                }
            });
    ...
    $(document).on("click",'#tolak_spm_verifikator',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_tambah_ks=' + '<?=$id_nomor_tambah_ks?>';
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_tambah_ks/proses_spm_tambah_ks')?>",
                data:data,
...
<form action="<?=site_url('rsa_tambah_ks/cetak_spm')?>" id="form_spm" method="post" style="display: none"  >
    <input type="text" name="dtable_2" id="dtable_2" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_tambah_ks == 'SPM-DRAFT-KPA'){ ?>
