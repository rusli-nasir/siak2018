...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                ...
                if(confirm('Apakah anda yakin ?')){
                    ...
                    var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_tambah_ks_spm['nom'])?$detail_tambah_ks_spm['nom']:''?>' + '&deskripsi=' + 'KS <?=$alias?>' + '&nominal=' + jumlah_bayar + '&id_nomor_tambah_ks=' + '<?=$id_nomor_tambah_ks?>'  ;
                    $.ajax({
                        type:"POST",
                        url :"<?=site_url('rsa_tambah_ks/proses_final_tambah_ks')?>",
                        data:data,
        ...
        //                        $('#myModalKuitansi').modal('show');
                                if(data=='sukses'){
                                    window.location = '<?=site_url('rsa_tambah_ks/daftar_spm_kbuu/').$cur_tahun.'/'.$kd_unit?>';
                                }
        //                        
                        }
                    });
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_tambah_ks=' + '<?=$id_nomor_tambah_ks?>';
            $.ajax({
                type:"POST",
							...
							<li>Kebenaran perhitungan dan isi tertuang dalam SPP ini menjadi tanggung jawab Bendahara Pengeluaran sepanjang sesuai dengan bukti-bukti pengeluaran yang telah ditandatangani oleh PPPK atau PPK</li>
                                ...
                                <tr>
                                    ...
                                    <td colspan="2" style="vertical-align: top;line-height: 16px;padding-left: 10px;">
                                        <?php if(isset($detail_tambah_ks['nom'])){ ?>
                                            <?php if($detail_tambah_ks['nom'] >= 100000000){ ?>
                                            Setuju dibayar : <br>
...
<form action="<?=site_url('rsa_tambah_ks/cetak_spm')?>" id="form_spm" method="post" style="display: none"  >
    <input type="text" name="dtable_2" id="dtable_2" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_tambah_ks == 'SPM-FINAL-VERIFIKATOR'){ ?>
