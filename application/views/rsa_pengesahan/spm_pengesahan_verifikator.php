...
$('#myModalKonfirm').on('hidden.bs.modal', function (e) {
    ...
    $(document).on("click",'#proses_spm_verifikator',function(){
        ...
        if(confirm('Apakah anda yakin ?')){
                ...
                var data = 'proses=' +'SPM-FINAL-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM'  + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' ;
                $.ajax({
                    type:"POST",
    ...
    //                        $('#myModalKuitansi').modal('show');
                            if(data=='sukses'){
                                // location.reload();
                                window.location = '<?=site_url('rsa_'.$jenis.'/daftar_spm_verifikator/').$cur_tahun.'/'.$kd_unit?>';
                            }
    //                        
                    }
                });
        ...
        //                     var data = 'proses=' +'SPM-FINAL-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM'  + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' + '&tgl_proses=' + r ;
        //                     $.ajax({
        //                         type:"POST",
        //                         url :"<?=site_url('rsa_'.$jenis.'/proses_spm_'.$jenis)?>",
        //                         data:data,
    ...
    $(document).on("click",'#tolak_spm_verifikator',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' ;
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_'.$jenis.'/proses_spm_'.$jenis)?>",
                data:data,
...
<div id="page-inner">
    ...
    <?php $stts_bendahara = 'done'; $stts_ppk = 'done'; $stts_kpa = 'done' ; $stts_verifikator = 'active';  ?>   
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                    ...
                    <button type="button" class="btn btn-warning" id="proses_spm_verifikator"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Setujui SPM</button>
