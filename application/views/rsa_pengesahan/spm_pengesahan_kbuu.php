...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(deskripsi.trim()==''){
                alert('Silahkan isi deskripsi pencairan terlebih dahulu');
            }else{
                ...
                //     alert('Mohon maaf, tgl proses harap diisi');
                    ...
                    if(confirm('Apakah anda yakin ?')){
                        ...
                        var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_spm['nom'])?$detail_spm['nom']:''?>' + '&deskripsi=' + deskripsi + '&nominal=' + jumlah_bayar + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&jumlah_pengembalian=' + '<?=$detail_spm['pengembalian']?>' + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' ;
                        $.ajax({
                            type:"POST",
                            url :"<?=site_url('rsa_'.$jenis.'/proses_final_'.$jenis)?>",
                            data:data,
            ...
            //                        $('#myModalKuitansi').modal('show');
                                    if(data=='sukses'){
                                        window.location = '<?=site_url('rsa_'.$jenis.'/daftar_spm_kbuu/').$cur_tahun.'/'.$kd_unit?>';
                                    }
            //                        
                            }
                        });
    ...
    <?php if(($jenis == 'ks_nihil')||($jenis == 'tup_nihil')||($jenis == 'gup_nihil')): ?>


    $(document).on("click",'#btn-proses-nihil',function(){
        if($('input[name="kd_akun_kas_pengembalian"]:checked').length > 0){
            var kd_akun_kas = $("input[name='kd_akun_kas_pengembalian']:checked").val();
            var jumlah_bayar = string_to_angka($('#jumlah_bayar').html());
            // var saldo_kas = string_to_angka($('#h_input_pengembalian_' + kd_akun_kas).val());
            var deskripsi = $('#deskripsi_pengembalian').val();

            var pengembalian = string_to_angka($('#h_input_pengembalian_' + kd_akun_kas).val());
...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(deskripsi.trim()==''){
                ...
                // }else{
                    if(confirm('Apakah anda yakin ?')){
                        ...
                        var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_spm['nom'])?$detail_spm['nom']:''?>' + '&deskripsi=' + deskripsi + '&nominal=' + jumlah_bayar + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&jumlah_pengembalian=' + '<?=$detail_spm['pengembalian']?>' + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' + '&nomor_trx_spm_cair=' + '<?=$kas_undip->no_spm?>' ;
                        $.ajax({
                            type:"POST",
                            url :"<?=site_url('rsa_'.$jenis.'/proses_final_'.$jenis)?>",
                            data:data,
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' + '&tgl_proses=' + '<?=date("Y-m-d")?>';
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_'.$jenis.'/proses_spm_'.$jenis)?>",
                data:data,
                success:function(data){
//                        console.log(data)
//                        $('#no_bukti').html(data);
...
<div id="page-inner">
    ...
    <?php $stts_bendahara = 'done'; $stts_ppk = 'done'; $stts_kpa = 'done' ; $stts_verifikator = 'done'; $stts_kbuu = 'active' ?>   
                ...
                <tr>
                                    <td colspan="5" style="line-height: 16px;border-bottom: none;border-top: none;">
                                        <ol style="list-style-type: lower-alpha;margin-top: 0px;margin-bottom: 0px;" >
                                            <li>Jumlah pembayaran yang diminta : Rp. <span id="jumlah_bayar_spp" style='mso-number-format:"\@";'><?php echo isset($detail_spm['nom'])?number_format($detail_spm['nom'], 0, ",", "."):''; ?></span>,-<br>
                                                &nbsp;&nbsp;&nbsp;(Terbilang : <b><span id="terbilang_spp"><?php echo isset($detail_spm['terbilang'])?ucwords($detail_spm['terbilang']):''; ?></span></b>)</li>
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                    <?php if(($jenis=='gup_nihil')||($jenis=='tup_nihil')||($jenis=='ks_nihil')): ?>
                        <button type="button" class="btn btn-warning" id="" data-toggle="modal" data-target="#myModalKasPengembalian"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Setujui SPM</button>
                    <?php else: ?>
                        <button type="button" class="btn btn-warning" id="" data-toggle="modal" data-target="#myModalKas"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Setujui SPM</button>
                    <?php endif; ?>
                    <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#myModalTolakSPMPPK"><span class="glyphicon glyphicon-remove-sign" aria-hidden="true"></span> Tolak SPM</button>
          ...
          <div class="modal-body">
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                  ...
                  <p><b><?=$ku->nama_akun?></b></p>
                  <div class="input-group">
                    <span class="input-group-addon">
                      <input type="radio" aria-label="" rel="" class="rdo_tambah_ks" name="kd_akun_kas" value="<?=$ku->kd_akun_kas?>">
                    </span>
                      <input type="hidden" value="<?=$ku->saldo?>" name="h_input_<?=$ku->kd_akun_kas?>" id="h_input_<?=$ku->kd_akun_kas?>" />
                      <input type="text" class="form-control" aria-label="" value="<?=$ku->kd_akun_kas?> [ Saldo : Rp. <?=number_format($ku->saldo, 0, ",", ".")?>,- ]" readonly="readonly">
                  </div>
                  </div>
                  <!-- /input-group -->
                </div><!-- /.col-lg-6 -->
             ...
             <div class="row">
                <div class="col-md-12">
                    <p>Keterangan Pencairan :</p>
                    <p>
                        <div class="form-group">
                        <textarea class="form-control" id="deskripsi" name="deskripsi" placeholder="Keterangan Pencairan"></textarea>
                        </div>
                    </p>
              </div>
             </div>
...
<!-- POP UP PILIH PENCAIRAN -->
<div class="modal" id="myModalKasPengembalian" tabindex="-1" role="dialog" aria-labelledby="myModalKas">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title" id="myModalLabelPengembalian">Konfirmasi</h4>
          </div>
          <div class="modal-body">
            <div class="form-group">
            <label for="exampleInputEmail1">Kas yang dikebalikan :</label>
            <div class="row">
                <div class="col-md-12">
                    <div class="alert alert-warning">
                        <?php $ku = $kas_undip ; ?>
                      <p><b><?=$ku->nama_akun?></b></p>
                            ...
                            <span class="input-group-addon">
                              <input type="radio" aria-label="" checked="checked" rel="" class="rdo_tambah_ks_pengembalian" name="kd_akun_kas_pengembalian" value="<?=$ku->kd_akun_kas?>">
                            </span>
                              <input type="text" class="form-control" aria-label="" value="<?=$ku->kd_akun_kas?> [ Saldo : Rp. <?=number_format($kas_undip_saldo, 0, ",", ".")?>,- ] " readonly="readonly">
                          </div><!-- /input-group -->
                      </div>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <p >
                        <div class="alert alert-warning">
                            <div class="row">
                                <div class="col-md-3">Permintaan </div><div class="col-md-1">:</div><div class="col-md-3" style="text-align:right;"><b>Rp. <?=number_format($ku->kredit, 0, ",", ".")?></b></div><div class="col-md-5">&nbsp;</div>
                            </div>
                            <div class="row">
                                <div class="col-md-3">SPJ </div><div class="col-md-1">:</div><div class="col-md-3" style="text-align:right"><b>Rp. <?=number_format($detail_spm['nom'], 0, ",", ".")?></b></div><div class="col-md-5">&nbsp;</div>
                            </div>
                            <div class="row">
                                <div class="col-md-3">Pengembalian </div><div class="col-md-1">:</div><div class="col-md-3" style="text-align:right"><b>Rp. <?=number_format(($ku->kredit - $detail_spm['nom']), 0, ",", ".")?></b></div><div class="col-md-5">&nbsp;</div>
                                <input type="hidden" value="<?=($ku->kredit - $detail_spm['nom'])?>" name="h_input_pengembalian_<?=$ku->kd_akun_kas?>" id="h_input_pengembalian_<?=$ku->kd_akun_kas?>" />
                            </div>
                        </div>
                    </p>
              </div>
             ...
             <div class="row">
                <div class="col-md-12">
                    ...
                    <p>
                        <div class="form-group">
                        <textarea class="form-control" id="deskripsi_pengembalian" name="deskripsi_pengembalian" placeholder="Keterangan Pencairan"></textarea>
                        </div>
                    </p>
              </div>
             </div>
          ...
          <div class="modal-footer">
         <button type="button" class="btn btn-success" id="btn-proses-nihil" ><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Pilih</button>
         <button type="button" class="btn btn-danger" data-dismiss="modal"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span> Batal</button>
          </div>
        </div>
    </div>
