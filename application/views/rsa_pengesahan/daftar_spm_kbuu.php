	...
	$(document).ready(function(){
      ...
      $('#spm_tab a[href="' + hash + '"]').tab('show');
		...
		$(document).on("click","#pilih_tahun",function(){
                            ...
                            window.location = "<?=site_url('rsa_'.$jenis.'/daftar_spm_kbuu')?>/" + $("#tahun").val() + '/' + '<?=$kode_unit?>' ;
  ...
  <!-- Tab panes -->
	...
	?>
					<tr >
						...
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_'.$jenis.'/spm_'.$jenis.'_kbuu/').urlencode(base64_encode($value->str_nomor_trx_spp))?>#spm">lihat</a> ]</b></td>
						<?php else:?>
