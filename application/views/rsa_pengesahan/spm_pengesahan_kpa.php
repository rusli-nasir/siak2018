...
$('#myModalKonfirm').on('hidden.bs.modal', function (e) {
    ...
    $(document).on("click",'#proses_spm_kpa',function(){
        ...
        //                 if(confirm("Yakin ?")){
                            ...
                            var data = 'proses=' + 'SPM-DRAFT-KPA' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' ;
                ...
                //                        $('#myModalKuitansi').modal('show');
                                        if(data=='sukses'){
                                            window.location = "<?=site_url('rsa_'.$jenis.'/daftar_spm_kpa')?>";
                                        }
                //                        
                                }
                            });
    ...
    $(document).on("click",'#tolak_spm_kpa',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KPA' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>')  + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' + '&tgl_proses=' + '<?=date("Y-m-d")?>' ;
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_'.$jenis.'/proses_spm_'.$jenis)?>",
                data:data,
...
<div id="page-inner">
    ...
    <?php $stts_bendahara = 'done'; $stts_ppk = 'done';  $stts_kpa = 'active'; ?>   
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                    ...
                    <button type="button" class="btn btn-warning" id="proses_spm_kpa"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Setujui SPM</button>
