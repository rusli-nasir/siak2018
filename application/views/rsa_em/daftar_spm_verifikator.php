      ...
      $('#spm_tab a[href="' + hash + '"]').tab('show');
		...
		$(document).on("click","#pilih_tahun",function(){
                        ...
                        // var type = window.location.hash.substr(1);
                            ...
                            // window.location = "<?=site_url("rsa_em/daftar_spm_verifikator")?>/" + $("#tahun").val() + '#' + type;

                            window.location = "<?=site_url("rsa_em/daftar_spm_verifikator")?>/" + $("#tahun").val() + '/' + '<?=$kode_unit?>' ;
  ...
  <!-- Tab panes -->
	...
	?>
					<tr >
						...
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_em/spm_em_verifikator/').urlencode(base64_encode($value->str_nomor_trx_spp))?>">lihat</a> ]</b></td>
						<?php else:?>
