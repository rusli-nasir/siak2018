...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                ...
                if(confirm('Apakah anda yakin ?')){
                    ...
                    var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_em_spm['nom'])?$detail_em_spm['nom']:''?>' + '&deskripsi=' + 'EM <?=$alias?>' + '&nominal=' + jumlah_bayar + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&jumlah_pengembalian=' + '<?=$detail_em['pengembalian']?>' + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&id_nomor_em=' + '<?=$id_nomor_em?>'  + '&tgl_proses=' + tgl_proses ;
                    $.ajax({
                        type:"POST",
                        url :"<?=site_url('rsa_em/proses_final_em')?>",
                        data:data,
        ...
        //                        $('#myModalKuitansi').modal('show');
                                if(data=='sukses'){
                                    window.location = '<?=site_url('rsa_em/daftar_spm_kbuu/').$cur_tahun.'/'.$kd_unit?>';
                                }
        //                        
                        }
                    });
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_em=' + '<?=$id_nomor_em?>' + '&tgl_proses=' + '<?=date("Y-m-d")?>';
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_em/proses_spm_em')?>",
                data:data,
                success:function(data){
//                        console.log(data)
//                        $('#no_bukti').html(data);
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_em == 'SPM-FINAL-VERIFIKATOR'){ ?>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row alert-warning" style="padding: 5px;" >
                    ...
                    <div class="col-sm-9">
                        <input type="text" class="form-control validate[required]" id="tgl_proses" readonly="readonly" placeholder="TGL APPROVED OLEH KBUU" value="<?=date('Y-m-d')?>"> <!-- <?=date('Y-m-d')?> -->
                    </div>
                </div>
            <div class="form-group">
