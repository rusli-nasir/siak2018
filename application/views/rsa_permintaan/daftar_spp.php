...
//                            var sumber_dana = $('#sumber_dana').val();
			...
			if((status.indexOf("SPM-FINAL-KBUU") >= 0)&&(status!='')){
				var no_spm = $('#table-spp tbody tr:first td:nth-child(3)').text();
				$.ajax({
                        type:"POST",
                        url :"<?=site_url("rsa_tup_nihil/check_tup_nihil_done")?>",
                        data:'no_spm=' + no_spm ,
                        success:function(data){

							if(data=='ok'){

								$('#createspp').removeAttr('disabled');
...
<div id="page-wrapper">
	...
	?>
					<tr>
						...
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_'.$jenis.'/spp_'.$jenis.'_lihat/').urlencode(base64_encode($value->str_nomor_trx_spp))?>">Lihat</a> ]<?php if(($jenis=='ks')&&(!empty($value->str_nomor_trx_spm))&&($value->posisi=='SPM-FINAL-KBUU')):?><hr><span class="badge" style="border: 1px solid #999;background-color:#f2ff04;">_ <a href="<?=site_url('kuitansi/daftar_kuitansi/KS/'.$cur_tahun.'/'.$kode_unit_subunit.'/'.urlencode(base64_encode($value->str_nomor_trx_spm)))?>">Nihil</a> _<?php endif; ?></b></td>
					</tr>
	<?php
