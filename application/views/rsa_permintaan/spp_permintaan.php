...
$(document).ready(function(){
        ...
        <?php if($detail_permintaan['nom'] == '0' ): ?>
    ...
    $(document).on("click",'#btn_penyesuaian',function(){
        ...
        if($("#simpan_rincian_spp").validationEngine("validate")){
            ...
            $('#id_akun_belanja').text($('#kode_akun6d').val());

            $.get("<?=site_url('rsa_akun/get_akun_by_id')?>/" + $('#kode_akun6d').val() , '' , function (data) {

              var data = $.parseJSON(data);
              $('#akun_belanja_nama5d').text(data.nama_akun5digit);
              $('#akun_belanja_kode5d').text(data.kode_akun5digit);
              $('#akun_belanja_nama').text(data.nama_akun);
              $('#akun_belanja_kode').text(data.kode_akun);
            ...
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_'.$jenis.'/get_next_nomor_spp')?>",
                data:'' ,
		...
		<table id="table_spp" style="font-family:arial;font-size:12px; line-height: 21px;border-collapse: collapse;width: 900px;border: 1px solid #000;background-color: #FFF;" cellspacing="0" border="1" cellpadding="0" >
			<tbody>
				...
				<tr>
                    ...
                    <span id="id_akun_belanja" style="display:none">0</span>
                    <b><span id="akun_belanja_nama5d">-</span></b><br/>
									<span id="akun_belanja_nama">-</span>
								</td>
								<td  style="padding-left: 10px;vertical-align: top;">
                    <b><span id="akun_belanja_kode5d">00000</span></b><br/>
									<span id="akun_belanja_kode">000000</span>
								</td>
...
<div class="modal" id="myModalTambahUP" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Form Permintaan</h4>
      </div>
    <div class="modal-body">
         <form class="form-horizontal" id="simpan_rincian_spp">
              ...
              <div class="form-group">
                  <label class="col-sm-3 control-label">Pilih Akun</label>
                  <div class="col-sm-9">

                    <?php if(!empty($list_akun)){
                       $options = array(); 
                       // $options_5d = array(); 
                       foreach($list_akun as $rr){
                          // if(in_array($rr->kode_akun5digit, $options_5d)){
                          //   $options_5d[] = $rr->kode_akun5digit;
                          // }
                          $options['[ ' .$rr->kode_akun5digit . ' ] ' .$rr->nama_akun5digit][$rr->id_akun_belanja] = '[ ' .$rr->kode_akun . ' ] ' .$rr->nama_akun ;
                        } 
                      } ?>


                    <?php echo form_dropdown('kode_akun6d', $options, '' , 'class="form-control validate[required]" id="kode_akun6d" placeholder=""'); ?>
      ...
      <div class="modal-footer">
        ...
        <a href="<?php echo site_url('rsa_'.$jenis.'/daftar_spp/');?>" class="btn btn-danger" id="btn_batal_1"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span> Batal</a>
