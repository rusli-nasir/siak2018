...
//            console.log(nominal + ' ' + saldo_kas);
            if(deskripsi.trim()==''){
                ...
                if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                    ...
                    if(confirm('Apakah anda yakin ?')){
                        ...
                        var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_permintaan_spm['nom'])?$detail_permintaan_spm['nom']:''?>' + '&deskripsi=' + deskripsi + '&nominal=' + jumlah_bayar + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' + '&kd_akun_kas_bendahara=' + $('#akun_belanja_kode').text()  ;
                        $.ajax({
                            type:"POST",
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>';
            $.ajax({
                type:"POST",
			...
			<tbody>
				...
				<tr>
                                    <td colspan="5" style="line-height: 16px;border-bottom: none;border-top: none;">
                                        <ol style="list-style-type: lower-alpha;margin-top: 0px;margin-bottom: 0px;" >
                                            <li>Jumlah pembayaran yang diminta : Rp. <span id="jumlah_bayar"><?=number_format($detail_permintaan_spm['nom'], 0, ",", ".")?></span>,-<br>
                                                ...
                                                <li>Nama Penerima : <span id="penerima"><?=isset($detail_pic_spm->penerima)?$detail_pic_spm->penerima:'-'?></span></li>
							...
							<tr>
								...
								<td style="text-align: right;padding-right: 10px;">
									Rp. <?php echo isset($detail_permintaan_spm['nom'])?number_format($detail_permintaan_spm['nom'], 0, ",", "."):''; ?>
								</td>
								<td>
									&nbsp;
								</td>
                ...
                <div class="col-md-12">
                  <div class="input-group">
                    <span class="input-group-addon">
                      ...
                      <input type="text" class="form-control" aria-label="" value="<?=$ku->kd_akun_kas?> [ Rp. <?=number_format($ku->saldo, 0, ",", ".")?>,- ] " readonly="readonly">
                  </div><!-- /input-group -->
                </div><!-- /.col-lg-6 -->
