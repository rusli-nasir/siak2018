...
//                        
    ...
    $(document).on("click",'#tolak_spm_ppk',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KPA' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&id_nomor_<?=$jenis?>=' + '<?=$id_nomor?>' ;
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_'.$jenis.'/proses_spm_'.$jenis)?>",
                data:data,
...
<form action="<?=site_url('rsa_tambah_ks/cetak_spm')?>" id="form_spm" method="post" style="display: none"  >
    <input type="text" name="dtable_2" id="dtable_2" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                    ...
                    <button type="button" class="btn btn-warning" id="proses_spm_ppk"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Setujui SPM</button>
