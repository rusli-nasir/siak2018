...
//            in_all = 0;
            ...
            $('#myModalKuitansi2 [class^="sub_tot_bruto_"').each(function(){
                sum_tot_bruto = sum_tot_bruto + parseFloat(string_to_angka($(this).html()));
            });
            ...
            $('#myModalKuitansi2 [class^="sub_tot_netto_"').each(function(){
                ...
                var sub_tot_bruto = parseFloat(string_to_angka($('#myModalKuitansi2 .sub_tot_bruto_' + nrel).text()));
                var pajak = parseFloat(string_to_angka($('#myModalKuitansi2 .row_pajak_nom_' + nrel).text()));
 ...
 * /
            ...
            $('[class^="sub_tot_netto_"').each(function(){
                var nrel = $(this).attr('rel');
                var sub_tot_bruto = parseFloat(string_to_angka($('.sub_tot_bruto_' + nrel).text()));
                var pajak = parseFloat(string_to_angka($('.row_pajak_nom_' + nrel).text()));
                var sub_tot_netto = sub_tot_bruto - pajak;
                $(this).text(angka_to_string(sub_tot_netto));
                sum_tot_netto = sum_tot_netto + sub_tot_netto ;
...
//            ck_121130010101010101521213001
            ...
            var tot_select = 0 ;
            var saldo_kas =  parseFloat(string_to_angka($('#saldo_kas').text()));
            $('[class^="ck_"]').each(function(){
                var relck = $(this).attr('rel');
                if(($(this).is(':checked'))&&($(this).is(':enabled'))){
                    var sub_tot_select = $('#td_sub_tot_' + relck).text();
                        tot_select = tot_select + parseFloat(string_to_angka(sub_tot_select));
                }
            });
...
//                alert('Mohon maaf jumlah saldo UP anda tidak mencukupi !');
                bootbox.alert({
                    size: "small",
                    title: "Perhatian",
                    message: 'Maaf jumlah saldo UP anda cukup !',
                    animate:false,
                });
                return false;
            }
...
//                $('#pj_dpp_' + rel).attr('disabled','disabled');
        ...
        $(document).on('click', '#btn-submit-pajak', function(){
            ...
            var netto = parseFloat(string_to_angka(bruto)) - parseFloat(string_to_angka(total_pajak));
...
//            var pj_p_nilai = [];
            ...
            $('.sub_tot_pajak').each(function(){
                sub_tot_pajak = sub_tot_pajak + parseFloat(string_to_angka($(this).text())) ;
            });
            $('.sum_tot_pajak').html(angka_to_string(sub_tot_pajak));

            var sum_tot_netto = 0 ;
            $('[class^="sub_tot_netto_"').each(function(){
                sum_tot_netto = sum_tot_netto + parseFloat(string_to_angka($(this).html()));
            });
...
function get_total_pajak(){
    ...
    $('[id^="pj_nilai_"]').each(function(){
        ...
        var nilai = parseFloat(string_to_angka(nilai_));
        total = total + nilai ;
    });
	...
	<td>
            <span class="label badge-gup">&nbsp;</span> : GUP &nbsp;&nbsp;<span class="label badge-tup">&nbsp;</span> : TUP &nbsp;&nbsp;<span class="label badge-lp">&nbsp;</span> : LS-PEGAWAI &nbsp;&nbsp;<span class="label badge-l3">&nbsp;</span> : LS-PIHAK-3 &nbsp;&nbsp;<span class="label badge-ks">&nbsp;</span> : KERJA-SAMA        </td>
						...
						?>
							...
							<button type="button" class="btn btn-warning" id="btn-spp2" rel="" disabled="disabled" ><span class="glyphicon glyphicon-duplicate" aria-hidden="true"></span> Buat SPP LSP3</button>
...
<div class="modal" id="modalGedang" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
              <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
              <h4 class="modal-title" id="myModalLabel"><i class="glyphicon glyphicon-exclamation-sign"></i>&nbsp;&nbsp;&nbsp;Isian SPP Khusus :</h4>
          </div>
          <form id="isian_spp_langsung">
            <div class="modal-body" style="margin:15px;padding:0px;padding-bottom: 15px;">
              <div class="row">
        				<div class="col-md-12">
        					<div class="col-md-12">
	    						<div class="form-group">
	    							<label for="daftar_kontrak">Uraian SPP:</label>
	    							<input type="hidden" name="kode_usulan_belanja" id="kode_usulan_belanja" value=""/>
	    							<input type="hidden" name="kode_akun_tambah" id="kode_akun_tambah" value=""/>
	    							<textarea name="uraian" class="form-control input-sm" id="uraian"></textarea>
	    						</div>
	    					</div>
    						<div class="col-md-12 alert-danger">
	    						<div class="col-md-6">
		    						<div class="form-group">
		    							<label for="penerima_uang">Penerima Uang:</label>
		    							<input type="text" class="form-control input-sm" name="penerima_uang" id="penerima_uang" value=""/>
		    						</div>
	    						</div>
	    						<div class="col-md-6">
		    						<div class="form-group">
		    							<label for="penerima_uang_nip">NIP Penerima Uang:</label>
		    							<input type="text" class="form-control input-sm" name="penerima_uang_nip" id="penerima_uang_nip" value=""/>
		    						</div>
		    					</div>
		    				</div>
		    				<div class="col-md-12 alert-success">
		    					<div class="col-md-6">
		    						<div class="form-group">
		    							...
		    							<input type="text" class="form-control input-sm" name="penerima_barang" id="penerima_barang" value=""/>
		    						</div>
		    					</div>
		    					<div class="col-md-6">
		    						<div class="form-group">
		    							<label for="penerima_barang_nip">NIP Penerima Barang:</label>
		    							<input type="text" class="form-control input-sm" name="penerima_barang_nip" id="penerima_barang_nip" value=""/>
		    						</div>
		    					</div>
		    				</div>
		    				<div class="col-md-12">
	    						<div class="form-group">
	    							...
	    							<select name="pppk" id="pppk" class="form-control input-sm">
									<?php if(!empty($pppk)){ ?>
	        						<?php 	foreach($pppk as $p){ ?>
	        							<option value="<?php echo $p->id; ?>"><?php echo $p->nm_lengkap; ?> (<?php echo $p->nomor_induk; ?>)</option>
	        						<?php 	} ?>
	        						<?php } ?>
	    							</select>
	    						</div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-success btn-sm jadikan_spp"><span class="glyphicon glyphicon-check" aria-hidden="true"></span> Buat SPP LSPH3</button>
              <button type="button" class="btn btn-default btn-sm" data-dismiss="modal"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span> Ndak</button>
            </div>
          </form>
        </div>
