<script type="text/javascript">
    $(document).ready(function(){
        ...
        $('.row_re_notif').each(function(){
            var rka = $(this).attr('data-kd-usulan');
            re_notif(rka);
        });
        ...
        $.ajax({
            ...
            success: function(data) {
               if(data == 'sukses'){
                    ...
                    localStorage.setItem("row_expand_akun6d", ".data_rsa_detail_"+kode_usulan_belanja);
                    localStorage.setItem("row_focus", "#deskripsi_"+kode_usulan_belanja);
                    usulan_tor_ajax_reload(kode_usulan_belanja.substr(6, 10),'<?php echo $sumber_dana ?>',kode_usulan_belanja);
               }
           }
       });
    }
...
function canceledit(kode){
    $('#usulan_tor_row_detail_'+kode).load('<?php echo site_url('tor/refresh_row_detail_to_validate')?>/'+ kode +'/<?php echo $sumber_dana ?>', function(){
        get_kode_akun_tambah();
    });
}
...
function usulan_tor_ajax_reload(kode,sumber_dana,kode_usulan_belanja){
    $.ajax({
        type: 'get',
        url: '<?php echo site_url('tor/usulan_tor_to_validate_ajax_reload');?>/'+kode+'/'+sumber_dana+'/<?php echo $this->uri->segment(6) ?>' ,
        data: '' ,
        success: function(data) {
            $('#o-table').html(data);
        },
...
function re_notif(kode_usulan_belanja){
    var notif_subunit = parseInt($('#notif_subunit_' + kode_usulan_belanja.substr(0,4)).html());
    var notif_sub_subunit = parseInt($('#notif_sub_subunit_' + kode_usulan_belanja.substr(0,6)).html());
    var notif4d = parseInt($('#notif4d_' + kode_usulan_belanja.substr(0,22)).html());
    var notif5d = parseInt($('#notif5d_' + kode_usulan_belanja.substr(0,23)).html());
    var notif6d = parseInt($('#notif6d_' + kode_usulan_belanja.substr(0,24)).html());
    ...
    if (notif_subunit < 1) {
        $('#notif_subunit_' + kode_usulan_belanja.substr(0,4)).hide();
    }else{
        $('#notif_subunit_' + kode_usulan_belanja.substr(0,4)).html(notif_subunit);
    }
    if (notif_sub_subunit < 1) {
        $('#notif_sub_subunit_' + kode_usulan_belanja.substr(0,6)).hide();
    }else{
        $('#notif_sub_subunit_' + kode_usulan_belanja.substr(0,6)).html(notif_sub_subunit);
    }
    if (notif4d < 1) {
        $('#notif4d_' + kode_usulan_belanja.substr(0,22)).hide();
    }else{
        $('#notif4d_' + kode_usulan_belanja.substr(0,22)).html(notif4d);
    }
    if (notif5d < 1) {
        $('#notif5d_' + kode_usulan_belanja.substr(0,23)).hide();
    }else{
        $('#notif5d_' + kode_usulan_belanja.substr(0,23)).html(notif5d);
    }
    if (notif6d < 1) {
        $('#notif6d_' + kode_usulan_belanja.substr(0,24)).hide();
    }else{
        $('#notif6d_' + kode_usulan_belanja.substr(0,24)).html(notif6d);
    }
}
</script>
<?php
