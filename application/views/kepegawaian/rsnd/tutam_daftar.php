        ...
        <p class="text-right">
		  ...
		  <div class="col-md-12 result_data">
        	...
        	<div class="col-md-12" style="background-color:#fff;padding:3px;">
              ...
              Bulan <?php echo $this->cantik_model->wordMonth($_SESSION['tutam_rsnd']['bulan']); ?> Tahun <?php if(!isset($_SESSION['tutam_rsnd']['tahun'])){echo $cur_tahun;}else{echo $_SESSION['tutam_rsnd']['tahun'];} ?>
            </h4>
...
?>
        				<tr><td colspan="9" class="alert alert-danger text-center">Tidak ada data Insentif Tugas Tambahan untuk <?php echo $this->cantik_model->wordMonth($_SESSION['tutam_rsnd']['bulan']); ?> <?php echo $_SESSION['tutam_rsnd']['tahun']; ?></td></tr>
<?php
				}
*/
?>
...
<script type="text/javascript">
	function load_list(){
		$('#load_list').load("<?php echo site_url('tutam_rsnd/daftar_ajax'); ?>");
	}
	...
	$(function(){
		...
		$('.home').on('click', function(e){
			window.location = '<?php echo site_url('tutam_rsnd'); ?>';
		});

		$('.cetak_daftar').on('click', function(e){
			window.location = '<?php echo site_url('tutam_rsnd/daftar_cetak'); ?>';
		});
		...
		$(document).on('click', '.hapus_daftar', function(e){
			...
			if(a){
				$.post('<?php echo site_url('tutam_rsnd/hapus_daftar_tutam'); ?>',function(r){
					if(!$.isNumeric(r)){
		...
		$(document).on('click','.trash',function(){
			...
			if(a){
				var id = $(this).attr('id');
				$.post('<?php echo site_url('tutam_rsnd/tutam_proses'); ?>',{'id':id, 'act':'tutam_hapus_single'},function(data){
					if(data!='1'){
