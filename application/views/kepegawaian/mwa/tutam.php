<!-- <link type="text/css" href="<?php echo base_url(); ?>assets/kepeg/select2/select2.min.css"/> -->
<style type="text/css">
	._update{
...
<script type="text/javascript">
	function mwa_ajax(){
		$('#daftar-mwa-ajax').load("<?php echo site_url('tutam_mwa/daftar_mwa_ajax'); ?>");
	}
	...
	function selectItem(id){
		...
		$('#yang_dipilih').html('');
		$('#pegid2').val(id);
		$.post('<?php echo site_url('tutam_mwa/daftar_mwa_ajax_pegawai_id'); ?>',{'id':id},function(r){
			console.log(r);
			q = $.parseJSON(r);
			console.log(q);
			if(!$.isEmptyObject(q)){
	...
	$(function(){
		$('#form-tutam').on('submit',function(e){
			...
			if(a){
				...
				$.post('<?php echo site_url('tutam_mwa/tutam_proses'); ?>',$('#form-tutam').serialize(),function(data){
					if(data!='1'){
						$('#myModalMsg .body-msg-text').html(data);
						$('#myModalMsg').modal('show');
						return false;
					}else{
						window.location='<?php echo site_url('tutam_mwa/daftar'); ?>';
						return false;
					}
				});
			}
		...
		$('#form-tutam-lihat').on('submit',function(e){
			...
			$.post('<?php echo site_url('tutam_mwa/tutam_proses'); ?>',$('#form-tutam-lihat').serialize(),function(data){
				if(data!=1){
					$('#myModalMsg .body-msg-text').html(data);
					$('#myModalMsg').modal('show');
					return false;
		...
		$('#_add_mwa').on('submit',function(e){
			...
			if(confirm("Yakin akan menyimpan data?")){
				$.post('<?php echo site_url('tutam_mwa/daftar_mwa_ajax_add'); ?>',data,function(r){
				if(!$.isNumeric(r)){
		...
		$('#nama2').on('keyup',function(e){
			var obj = $(this);
			if(obj.val().length > 0){
				$.post('<?php echo site_url('tutam_mwa/daftar_mwa_ajax_pegawai'); ?>',{'q':obj.val()},function(r){
					$('#yang_dipilih').show();
					$('#yang_dipilih').html(r);
				});
			}else{
	...
	$(document).on('blur','._update',function(e){
		...
		$.post('<?php echo site_url('tutam_mwa/daftar_mwa_ajax_update'); ?>',{'field':field,'id':id,'value':value},function(r){
			if(!$.isNumeric(r)){
				$('#myModalMsg .body-msg-text').html(r);
				$('#myModalMsg').modal('show');
			}else{
	...
	$(document).on('click','._delete',function(e){
		...
		if(confirm("Hapus data ini?")){
			$.post('<?php echo site_url('tutam_mwa/daftar_mwa_ajax_delete'); ?>',{'id':id},function(r){
				if(!$.isNumeric(r)){
					$('#myModalMsg .body-msg-text').html(r);
					$('#myModalMsg').modal('show');
				}else{
		...
		<div class="row">
			<div class="col-md-12">
				<h4 class="page-header">
					<i class="glyphicon glyphicon-gift"></i>&nbsp;&nbsp;Data Insentif Tugas Tambahan (Majelis Wali Amanat) - <a href="<?php echo site_url('kepegawaian'); ?>" title="ke Menu Utama Kepegawaian">Kepegawaian #3</a>
				</h4>
				<p class="text-right">
					<button type="button" class="btn btn-default btn-flat btn-sm" data-toggle="modal" data-target="#mwa_tambah"><i class="fa fa-user-plus"></i>&nbsp;&nbsp;&nbsp;Tambah MWA</button>
					<button type="button" class="btn btn-default btn-flat btn-sm" data-toggle="modal" data-target="#mwa_daftar"><i class="glyphicon glyphicon-list"></i>&nbsp;&nbsp;&nbsp;Daftar MWA</button>
...
<div class="modal fade" id="kriteria_tutam" role="dialog" arialabelledby="myModalLabel">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form-tutam" action="<?php echo site_url('tutam_mwa/tutam_proses'); ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          ...
          <div class="row">
	              ...
	              <input type="text" class="form-control input-sm" name="tahun" value="<?php if(!isset($_SESSION['tutam_mwa']['tahun'])){echo $cur_tahun;}else{echo $_SESSION['tutam_mwa']['tahun'];} ?>" style="text-align:right;"/>
	            </div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
...
<div class="modal fade" id="kriteria_tutam_lihat" role="dialog" arialabelledby="myModalLabel">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form-tutam-lihat" action="<?php echo site_url('tutam_mwa/tutam_proses'); ?>" method="post" enctype="multipart/form-data">
        ...
        <div class="modal-body">
          <span class="message"></span>
        	<div class="row">
	              ...
	              <input type="text" class="form-control input-sm" name="tahun" id="tahun" value="<?php if(!isset($_SESSION['tutam_mwa']['tahun'])){echo $cur_tahun;}else{echo $_SESSION['tutam_mwa']['tahun'];} ?>" style="text-align:right;"/>
	            </div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
  ...
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
        ...
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span></button>
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-list"></i>&nbsp;&nbsp;&nbsp;Daftar Majelis Wali Amanat</h5>
        </div>
        <div class="modal-body">
          <span class="message"></span>
        ...
        <div class="modal-footer">
          <div class="btn-group pull-right">
            ...
            <button type="button" class="btn btn-default btn-sm _insert" data-toggle="modal" data-target="#mwa_tambah"><i class="fa fa-user-plus"></i>&nbsp;&nbsp;&nbsp;Tambah MWA</button>
          </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form id="_add_mwa">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span></button>
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-user-plus"></i>&nbsp;&nbsp;&nbsp;Tambah Majelis Wali Amanat</h5>
        </div>
        <div class="modal-body">
          <span class="message"></span>
			<div class="row">
        		...
        		<div class="col-md-4">
					...
					<div class="input-group">
						...
						<input type="text" name="det_tgs_tambahan" id="det_tgs_tambahan2" class="form-control input-sm"/>
					</div>
