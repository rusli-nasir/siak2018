		...
		<div class="row">
			<div class="col-md-12">
				<h4 class="page-header">
					<i class="glyphicon glyphicon-gift"></i>&nbsp;&nbsp;Data Insentif Tugas Tambahan - <a href="<?php echo site_url('modul_gaji'); ?>" title="ke Menu Utama Kepegawaian">Kepegawaian #3</a>
				</h4>
        <p class="text-right">
		  ...
		  <div class="col-md-12 result_data">
        	...
        	<div class="col-md-12" style="background-color:#fff;padding:3px;">
              ...
              Bulan <?php echo $this->cantik_model->wordMonth($_SESSION['tutam_mwa']['bulan']); ?> Tahun <?php if(!isset($_SESSION['tutam_mwa']['tahun'])){echo $cur_tahun;}else{echo $_SESSION['tutam_mwa']['tahun'];} ?>
            </h4>
...
?>
        				<tr><td colspan="9" class="alert alert-danger text-center">Tidak ada data Insentif Tugas Tambahan untuk <?php echo $this->cantik_model->wordMonth($_SESSION['tutam_mwa']['bulan']); ?> <?php echo $_SESSION['tutam_mwa']['tahun']; ?></td></tr>
<?php
				}
*/
?>
...
<script type="text/javascript">
	function load_list(){
		$('#load_list').load("<?php echo site_url('tutam_mwa/daftar_ajax'); ?>");
	}
	...
	$(function(){
		...
		$('.home').on('click', function(e){
			window.location = '<?php echo site_url('tutam_mwa'); ?>';
		});

		$('.cetak_daftar').on('click', function(e){
			window.location = '<?php echo site_url('tutam_mwa/daftar_cetak'); ?>';
		});

		$(document).on('click', '.hapus_daftar', function(e){
			var a=confirm('Hapus daftar ini?\nUntuk memproses daftar kembali, lakukan melalui menu proses data tugas tambahan.');
			if(a){
				$.post('<?php echo site_url('tutam_mwa/hapus_daftar_tutam'); ?>',function(r){
					if(!$.isNumeric(r)){
		...
		$(document).on('submit','#form-tutam-lihat',function(e){
			e.preventDefault();
		...
		$(document).on('click','.opsi-float-bar',function(){ $('#float-bar').toggle(); });

		$(document).on('click','.trash',function(){
			var a=confirm('Yakin akan menghapus data ini?');
			if(a){
				var id = $(this).attr('id');
				$.post('<?php echo site_url('tutam_mwa/tutam_proses'); ?>',{'id':id, 'act':'tutam_hapus_single'},function(data){
					if(data!='1'){
