<script type="text/javascript">
		...
		$(document).on('click','.duh_beranda',function(e){
			window.location='<?php echo site_url('uk_dosen'); ?>';
		});
		function reload_daftar(){
			$('.result_data').load('<?php echo site_url('uk_dosen/reload_daftar'); ?>',function(r){
				$('._tooltip').tooltip();
			});
		}
		$(function(){
		...
		$(document).on('click','.del_item',function(e){
			...
			if(a){
				$.post('<?php echo site_url('uk_dosen/uk_proses'); ?>',{'act':'uk_hapus_single','id':gendut},function(r){
					if(!$.isNumeric(r)){
		...
		$(document).on('click','.del_daftar',function(e){
			...
			if(a){
				$.post('<?php echo site_url('uk_dosen/uk_proses'); ?>',{'act':'uk_hapus_daftar'},function(r){
					if(!$.isNumeric(r)){
						$('#myModalMsg .body-msg-text').html(r);
						$('#myModalMsg').modal('show');
					}else{
		...
		$(document).on('click','.print_daftar',function(e){
			e.preventDefault();
			window.location = '<?php echo site_url('uk_dosen/daftar_cetak'); ?>';
		});
</script>
<style type="text/css">
	.scroll-150{
		...
		<div class="row">
			<div class="col-md-12">
				...
				<p class="text-right">
					...
					<button type="button" class="btn btn-default btn-flat btn-sm duh_liat_ah" data-toggle="modal" data-target="#kriteria_ikk_lihat"><i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;&nbsp;Lihat Daftar Uang Kinerja</button>
				</p>
			</div>
			<div class="col-md-12">
		  ...
		  <div class="col-md-12 result_data">
				...
				?>
				<p class="alert alert-warning text-center"><i class="glyphicon glyphicon-lamp"></i>&nbsp;&nbsp;Gunakan &nbsp;&nbsp;<span class="btn btn-sm btn-default"><i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;&nbsp;Lihat Daftar Uang Kinerja</span>&nbsp;&nbsp; untuk melihat proses data Uang Kinerja (UK) Dosen.</p>
				<?php
					// }
				?>
			</div>
