...
//                        
    ...
    $(document).on("click",'#tolak_spm_kpa',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-VERIFIKATOR' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>';
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_tup/proses_spm_tup')?>",
                data:data,
