...
$(document).on("click","#show",function(){
    if($("#form_").validationEngine("validate")){
        window.location = "<?=site_url('rsa_tambah_up/daftar_unit/')?>" + $('#tahun').val();
                                ...
                                <?php foreach($unit_usul as $i => $u){ ?>
                                    <?php if(($u->kode_unit == '41')||($u->kode_unit == '42')||($u->kode_unit == '43')||($u->kode_unit == '44')): ?>
                                        ...
                                        <tr rel="<?=$uu->kode_subunit?>" class="tr-unit warning" height="25px">
                                            ...
                                            <td style=""><!--<?php // setlocale(LC_ALL, 'id_ID.utf8'); echo empty($uu->tgl_proses)?'':strftime("%d %B %Y", strtotime($uu->tgl_proses)); ?>--> Jumlah : <span class="badge badge-danger"><?=$u->posisi_ver?></span> </td>
                                            <td align="center">
                                                <?php if($uu->posisi_ver>0): ?>
