<script type="text/javascript">
	$(document).on('click','.halaman',function(e){
		$.post('<?php echo site_url('kepeg_tutam/set_halaman'); ?>',{'id':$(this).attr('id')},function(data){
			if(data=='1'){
	...
	$(document).on('submit','#form_lihat',function(e){
		...
		$.post($(this).attr('action'),$(this).serialize(),function(data){
			if(data=='1'){
				...
				window.location='<?php echo site_url("kepeg_tutam/mungil"); ?>';
			}else{
				$('#myModalMsg .body-msg-text').html(data);
				$('#myModalMsg').modal('show');
			}
		...
		<div class="row">
			<div class="col-md-12">
				...
				<div class="text-right">
					<button type="button" class="btn btn-default btn-flat btn-sm" data-toggle="modal" data-target="#kriteria_lihat"><i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;&nbsp;Lihat Daftar Proses IKW TuTam</button>
					<button type="button" class="btn btn-primary btn-flat btn-sm" data-toggle="modal" data-target="#kriteria"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Proses IKW TuTam</button>
				</div>
...
<?php
	if($jt>0){
		...
		if(isset($_SESSION['tutam']['page'])){
			$i = (($_SESSION['tutam']['page']-1)*$this->tutam_model->_maxPage)+1;
		}
...
?>
										<tr>
											...
											<td></td>
											<td class="text-center"><?php echo $this->tutam_model->getStatus($v->status); ?></td>
											<td class="text-center"><?php echo $this->tutam_model->getJenisPeg($v->jnspeg); ?></td>
											<td><?php echo $v->npwp; ?></td>
											<td><?php echo $this->tutam_model->getRekening($v->id,2); ?></td>
											<td><button type="button" class="btn btn-primary btn-sm dt_bayar" title="daftar proses pembayaran insentif tugas tambahan" id="<?php echo $v->nip; ?>"><i class="glyphicon glyphicon-credit-card"</button></td>
										</tr>
<?php
			$i++;
		}
...
<?php
	...
	for($j=1;$j<=$page;$j++){
		$s = "";
		if(isset($_SESSION['tutam']['page']) && $_SESSION['tutam']['page']==$j){
			$s = " class=\"active\"";
		}elseif((!isset($_SESSION['tutam']['page']) && $j==1)){
			$s = " class=\"active\"";
		}
?>
...
<?php /*?><!-- /.modal-dialog untuk menambahkan item transaksi-->
<div class="modal fade" id="kriteria" tabindex="-1" role="dialog" arialabelledby="myModalLabel">
  <!-- /.modal-content -->
  <div class="modal-dialog modal-xl">
    <div class="modal-content">
      <form id="form_proses" action="<?php echo site_url('kepeg_tutam/proses_tutam'); ?>" method="post" enctype="multipart/form-data">
        <input type="hidden" id="act" name="act" value="proses1"/>
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">×</span></button>
          <h5 class="modal-title" id="myModalLabel"><i class="fa fa-spinner"></i>&nbsp;&nbsp;&nbsp;Kriteria Proses Data Tugas Tambahan</h5>
        </div>
          ...
          <div class="row">
		            ...
		            <select name="jnspeg" id="jnspeg" class="form-control input-sm">
		              <?php
										...
										foreach ($_jenispeg as $k => $v) {
											$_s = "";
											if(isset($_SESSION['tutam']['jnspeg']) && $_SESSION['tutam']['jnspeg']==$v[0]){
												$_s = " selected";
											}
									?>
		      ...
		      <div class="row">
	            ...
	            <?php
								...
								foreach ($stt as $k => $v) {
									$ch = "";
									if(isset($_SESSION['tutam']['status']) && in_array($k,$_SESSION['tutam']['status'])){
										$ch = " checked = \"checked\"";
									}
							?>
