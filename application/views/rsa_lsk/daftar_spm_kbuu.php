      ...
      $('#spm_tab a[href="' + hash + '"]').tab('show');
		...
		$(document).on("click","#pilih_tahun",function(){
                        ...
                        // var type = window.location.hash.substr(1);
                            ...
                            // window.location = "<?=site_url("rsa_lsk/daftar_spm_kbuu")?>/" + $("#tahun").val() + '#' + type;

                            window.location = "<?=site_url("rsa_lsk/daftar_spm_kbuu")?>/" + $("#tahun").val() + '/' + '<?=$kode_unit?>' ;
        ...
        <?php var_dump($_SESSION); ?>
            */
            ...
            string = $(this).text() ,substring = "SPM-FINAL-VERIFIKATOR";
            if(string.indexOf(substring) !== -1){
  ...
  <!-- Tab panes -->
	...
	?>
					<tr >
						...
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_lsk/spp_lsk_lihat/').urlencode(base64_encode($value->str_nomor_trx_spp))?>">lihat</a> ]</b></td>
						<?php elseif($value->posisi == 'SPM-FINAL-VERIFIKATOR'):?>
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_lsk/spm_lsk_kbuu/').urlencode(base64_encode($value->str_nomor_trx_spp))?>">lihat</a> ]</b></td>
						<?php else:?>
						<td class="text-center"><b>[ <a href="<?=site_url('/rsa_lsk/spm_lsk_lihat/').urlencode(base64_encode($value->str_nomor_trx_spp))?>">lihat</a> ]</b></td>
						<?php endif; ?>
					</tr>
	<?php
			}
