...
//            console.log(nominal + ' ' + saldo_kas);
            ...
            if(parseInt(saldo_kas) < parseInt(jumlah_bayar)){
                ...
                alert('Mohon maaf, tgl proses harap diisi');
            }else{
                if(confirm('Apakah anda yakin ?')){
                    
                    var data = 'proses=' + 'SPM-FINAL-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&tahun=' + '<?=$cur_tahun?>' +'&kd_unit=' + '<?=$kd_unit?>' + '&kd_akun_kas=' + kd_akun_kas + '&kredit=' + '<?php echo isset($detail_lsk_spm['nom'])?$detail_lsk_spm['nom']:''?>' + '&deskripsi=' + 'LS-KONTRAK <?=$alias?>' + '&nominal=' + jumlah_bayar + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&jumlah_pengembalian=' + '<?=$detail_lsk['pengembalian']?>' + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&id_nomor_lsk=' + '<?=$id_nomor_lsk?>' + '&tgl_proses=' + tgl_proses ;
                    $.ajax({
                        type:"POST",
                        url :"<?=site_url('rsa_lsk/proses_final_lsk')?>",
                        data:data,
        ...
        //                        $('#myModalKuitansi').modal('show');
                                if(data=='sukses'){
                                    window.location = '<?=site_url('rsa_lsk/daftar_spm_kbuu/').$cur_tahun.'/'.$kd_unit?>';
                                }
        //                        
                        }
                    });
    ...
    $(document).on("click",'#tolak_spm_kbuu',function(){
        if(confirm('Apakah anda yakin ?')){
            var data = 'proses=' + 'SPM-DITOLAK-KBUU' + '&nomor_trx=' + $('#nomor_trx_spm').html() + '&jenis=' + 'SPM' + '&ket=' + $('#ket').val() + '&rel_kuitansi=' + encodeURIComponent('<?=$rel_kuitansi?>') + '&rel_kuitansi_pengembalian=' + encodeURIComponent('<?=$rel_kuitansi_pengembalian?>') + '&kd_unit=' + '<?=$kd_unit?>' + '&tahun=' + '<?=$cur_tahun?>' + '&id_nomor_lsk=' + '<?=$id_nomor_lsk?>' + '&tgl_proses=' + '<?=date("Y-m-d")?>' ;
            $.ajax({
                type:"POST",
                url :"<?=site_url('rsa_lsk/proses_spm_lsk')?>",
                data:data,
                success:function(data){
//                        console.log(data)
//                        $('#no_bukti').html(data);
                        ...
                        <tr>
                            ...
                            <td colspan="5" style="border-left: none;border-top: none;">
                                <ol style="list-style: none;">
                                    <li>: <?=$detail_kontrak->nomor_kontrak?> &nbsp; <?=$detail_kontrak->tgl_kontrak?></li>
                                    <li>: <?=number_format($detail_kontrak->nilai_kontrak, 0, ",", ".")?></li>
                                    <li>: <?=number_format($detail_kontrak->nilai_kontrak_terbayar, 0, ",", ".")?></li>
                                    <li>: <?=$detail_kontrak->termin?></li>
                                    <li>: <?=$detail_kontrak->jenis_kegiatan?></li>
                                    <li>: <?=$detail_kontrak->nomor_bap?></li>
                                    <li>: <?=$detail_kontrak->nomor_bast?></li>
                                    <li>:</li>
                                </ol>
                            </td>
                        </tr>
		...
		<table id="table_spm" style="font-family:arial;font-size:12px; line-height: 21px;border-collapse: collapse;width: 900px;border: 1px solid #000;background-color: #FFF;" cellspacing="0" border="1" cellpadding="0" >
            <tbody>
                            ...
                            <tr style="">
                                ...
                                <tr style="border-top: none;">
                                    <td colspan="2" style="border-right: none;border-top:none;"><b>Tanggal  : <?php setlocale(LC_ALL, 'id_ID.utf8'); echo $tgl_spm==''?'':strftime("%d %B %Y", strtotime($tgl_spm)); ?></b></td>
                                    <td style="text-align: center;border-left: none;border-right: none;border-top:none;" colspan="2" >&nbsp;</td>
...
<form action="<?=site_url('rsa_up/cetak_spp')?>" id="form_spp" method="post" style="display: none"  >
    <input type="text" name="dtable" id="dtable" value="" />
            ...
            <div class="alert alert-warning" style="text-align:center">
                ...
                <?php if($doc_lsk == 'SPM-FINAL-VERIFIKATOR'){ ?>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" id="tolak_spm_kbuu">Submit</button>
...
<div class="modal" id="myModalKas" tabindex="-1" role="dialog" aria-labelledby="myModalKas">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
              <div class="row alert-warning" style="padding: 5px;" >
                    <label for="inputEmail3" class="col-sm-3 control-label">Tgl Approved</label>
                    <div class="col-sm-9">
                        <input type="text" class="form-control validate[required]" id="tgl_proses" readonly="readonly" placeholder="TGL APPROVED OLEH KBUU" value=""> <!--<?=date('Y-m-d')?>-->
                    </div>
                </div>
            <div class="form-group">
