<?php
	...
	class rsa_pup extends CI_Controller{
/* -------------- Constructor ------------- */
	...
	public function __construct(){ 
		parent::__construct();
			...
			$this->load->model(array('rsa_pup_model','akun_model','kas_undip_model','kas_bendahara_model'));//,'setting_pup_model'));
...
//                        $data['message']	= validation_errors();
		...
		function input_rsa_pup(){
			if($this->check_session->user_session() && $this->check_session->get_level()==13){
				...
				$data['main_menu']	= $this->load->view('main_menu','',TRUE);		
				//$subdata_rsa_pup['result_rsa_pup'] 		= $this->rsa_pup_model->search_rsa_pup();
				//$subdata['row_rsa_pup'] 			= $this->load->view("rsa_permintaan/row_rsa_pup",$subdata_rsa_pup,TRUE);
				$subdata['opt_unit_kepeg']		= $this->option->opt_unit_kepeg();
				$data['main_content'] 			= $this->load->view("rsa_permintaan/input_rsa_pup",$subdata,TRUE);
				$this->load->view('main_template',$data);
			}
			else{
                ...
                $id_max_nomor = $this->rsa_pup_model->get_nomor_spp_urut($this->check_session->get_unit(),$this->cur_tahun) ;

                $id_max_nomor = $id_max_nomor + 1 ;

                // echo  $id_max_nomor_pup; die;
                                
                                $dokumen = '';//$this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun,$id_max_nomor_pup);
                               
                               // echo $dokumen_pup ; die ;
...
//                                        'tgl_spp' => $rsa_user->tgl_spp,
                                    ...
                                    $nomor_trx_ = $this->rsa_pup_model->get_nomor_next_spp($this->check_session->get_unit(),$this->cur_tahun);

                                    setlocale(LC_ALL, 'id_ID.utf8');$bln = strftime("%h");$thn = strftime("%Y");  

                                    $nomor_trx = $nomor_trx_.'/'.$this->check_session->get_alias().'/'.'SPP-PUP'.'/'.strtoupper($bln).'/'.$thn;
                ...
                $id_max_nomor_pup = $this->rsa_pup_model->get_nomor_spp_urut($this->check_session->get_unit(),$this->cur_tahun) ;

                $id_max_nomor_pup = $id_max_nomor_pup + 1 ;

                // echo  $id_max_nomor_pup; die;
                                
                                $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun,$id_max_nomor_pup);
                               ...
                               // echo $dokumen_pup ; die ;
                                ...
                                if(($dokumen_pup == '')||($dokumen_pup == 'SPP-DITOLAK')||($dokumen_pup == 'SPM-DITOLAK-KPA')||($dokumen_pup == 'SPM-DITOLAK-VERIFIKATOR')||($dokumen_pup == 'SPM-DITOLAK-KBUU')){
                                    $subdata['detail_pup']    = array(
                                                                    'nom' => '0',
                                                                    'terbilang' => $this->convertion->terbilang('0'), 
...
//                                    var_dump($subdata['detail_pic']);die;
    ...
    //                                if(($dokumen_pup == '') || ($dokumen_pup == 'SPP-DITOLAK') || ($dokumen_pup == 'SPM-DITOLAK')){
    //                                    $subdata['siap_spp'] = 'ok';
    //                                }else{
    //                                    $subdata['siap_spp'] = 'no_ok';
    //                                }

                                    $subdata['tgl_spp'] = $this->rsa_pup_model->get_tgl_spp($this->check_session->get_unit(),$this->cur_tahun);
                                    ...
                                    $nomor_trx = $this->rsa_pup_model->get_nomor_spp($this->check_session->get_unit(),$this->cur_tahun); 
                                    $data_spp = $this->rsa_pup_model->get_data_spp($nomor_trx);
                                    $subdata['detail_pup']    = array(
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
    ...
    //                                }
                                ...
                                $subdata['id_nomor_pup'] = $id_max_nomor_pup ;
                                
                                $subdata['doc_pup'] = $dokumen_pup;
                                $subdata['nomor_spp'] = $nomor_trx;
                                $subdata['ket'] = $this->rsa_pup_model->lihat_ket($this->check_session->get_unit(),$this->cur_tahun);
                                
//                                $urut_spp_pup = $this->rsa_pup_model->get_urut($this->check_session->get_unit(),'SPP',$this->cur_tahun);
                                
                //$subdata['opt_unit_kepeg']        = $this->option->opt_unit_kepeg();
//                                var_dump($subdata['detail_pic']);die;
                $data['main_content']           = $this->load->view("rsa_permintaan/spumk_pup",$subdata,TRUE);
                $this->load->view('main_template',$data);
            } 
            else{
                redirect('welcome','refresh');  // redirect ke halaman home
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE);       
                                ...
                                $dokumen = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($url);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['id_nomor_pup'] = $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx);

                                $subdata['doc'] = $dokumen;
                                $subdata['nomor_spp'] = $nomor_trx;
                                $subdata['detail_ppk']  = $this->user_model->get_detail_rsa_user($kd_unit,'14');

                                $subdata['ket'] = $this->rsa_pup_model->lihat_ket_by_nomor_trx($nomor_trx);
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE);       
                                ...
                                $id_nomor = $this->rsa_pup_model->get_nomor_spp_urut_by_nomor_spp($nomor_trx_spp) ;
                                
                                
                                $dokumen = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($url);

                                $nomor_trx_spm = $this->rsa_pup_model->get_spm_by_spp($nomor_trx_spp);
                                ...
                                // SPP //
                                    ...
                                    $data_spp = $this->rsa_pup_model->get_data_spp($nomor_trx_spp);
...
//                                    echo $data_spp->tgl_spp; die
                                ...
                                // SPM //
                                    ...
                                    // $nomor_trx_spm = $this->rsa_pup_model->get_nomor_spm($this->check_session->get_unit(),$this->cur_tahun);  

                                     // $id_nomor_pup_spm = $this->rsa_pup_model->get_nomor_spm_urut_by_nomor_spp($nomor_trx_spp) ;
                                    
                                    // $nomor_trx_spm = $this->rsa_pup_model->get_nomor_spm_by_id($id_nomor_pup_spm);  
                                    
                                    $data_spm = $this->rsa_pup_model->get_data_spm($nomor_trx_spm);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                // $subdata['tgl_spm_kpa'] = $this->rsa_pup_model->get_tgl_spm_kpa($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_kpa'] = $this->rsa_pup_model->get_tgl_spm_kpa_by_spp($nomor_trx_spp);
                                
                                // $subdata['tgl_spm_verifikator'] = $this->rsa_pup_model->get_tgl_spm_verifikator($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_verifikator'] = $this->rsa_pup_model->get_tgl_spm_verifikator_by_spp($nomor_trx_spp);
                                
                                // $subdata['tgl_spm_kbuu'] = $this->rsa_pup_model->get_tgl_spm_kbuu($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_kbuu'] = $this->rsa_pup_model->get_tgl_spm_kbuu_by_spp($nomor_trx_spp);
                                
                                // $subdata['ket'] = $this->rsa_pup_model->lihat_ket($this->check_session->get_unit(),$this->cur_tahun);
                                // echo $this->rsa_pup_model->lihat_ket_by_nomor_trx($nomor_trx_spp); die ;
                                $subdata['ket'] = $this->rsa_pup_model->lihat_ket_by_nomor_trx($nomor_trx_spp);
...
//                                var_dump($subdata);die;
				...
				$data['main_menu']	= $this->load->view('main_menu','',TRUE);	
                                ...
                                $id_nomor = $this->rsa_pup_model->get_nomor_spp_urut_by_nomor_spp($str_nomor_trx_spp) ;
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                //////////
                                    ...
                                    $nomor_trx_ = $this->rsa_pup_model->get_nomor_next_spm($this->check_session->get_unit(),$this->cur_tahun);
...
//                                    $subdata['detail_pic']  = $this->user_model->get_detail_rsa_user($this->check_session->get_unit(),'13');
                                    ...
                                    $subdata['detail_verifikator']  = $this->rsa_pup_model->get_verifikator(substr($this->check_session->get_unit(),0,2));
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['tgl_spm_kpa'] = $this->rsa_pup_model->get_tgl_spm_kpa($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                                
                                $subdata['tgl_spm_verifikator'] = $this->rsa_pup_model->get_tgl_spm_verifikator($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                                
                                $subdata['tgl_spm_kbuu'] = $this->rsa_pup_model->get_tgl_spm_kbuu($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                ...
                //$subdata_rsa_pup['result_rsa_pup']        = $this->rsa_pup_model->search_rsa_pup();
//              $subdata['detail_pup']            = array(
//                                                                                    'nom' => $this->setting_pup_model->get_setting_pup($this->check_session->get_unit(),$this->cur_tahun),
//                                                                                    'terbilang' => $this->convertion->terbilang($this->setting_pup_model->get_setting_pup($this->check_session->get_unit(),$this->cur_tahun)), 
//                                                                                    
//                                                                                );
                                ...
                                $dokumen = 'SPP-FINAL' ; // $this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_pup);
...
//                                $subdata['detail_pic']  = $this->user_model->get_detail_rsa_user($this->check_session->get_unit(),'13');
                                ...
                                // $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun);

                                
                                
//                                $subdata['tgl_spm'] = $this->rsa_pup_model->get_tgl_spm($this->check_session->get_unit(),$this->cur_tahun);
                                
                                // $nomor_trx_spp = $this->rsa_pup_model->get_nomor_spp($this->check_session->get_unit(),$this->cur_tahun); 
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                /////////
                                    ...
                                    $nomor_trx_ = $this->rsa_pup_model->get_nomor_next_spm($this->check_session->get_unit(),$this->cur_tahun);
                                    setlocale(LC_ALL, 'id_ID.utf8');$bln = strftime("%h");$thn = strftime("%Y");
                                    $nomor_trx_spm = $nomor_trx_.'/'.$this->check_session->get_alias().'/'.'SPM-PUP'.'/'.strtoupper($bln).'/'.$thn;
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
				...
				//$subdata_rsa_pup['result_rsa_pup'] 		= $this->rsa_pup_model->search_rsa_pup();
                                ...
                                // echo  $id_nomor_pup; die;
                                
                                $dokumen = 'SPM-DRAFT-PPK';//this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_pup);
                                ...
                                // $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_pup);
                                
                                $nomor_trx_spp = $str_nomor_trx_spp ; // $this->rsa_pup_model->get_nomor_spp_by_id($id_nomor_pup); 
...
//                            $subdata['unit_usul'] 		= $this->rsa_pup_model->get_pup_unit_usul_verifikator($user->id,$tahun);
                            $subdata['unit_usul'] 		= $this->rsa_pup_model->get_pup_unit_usul_verifikator($user->id,$tahun);
                            // echo '<pre>';var_dump($subdata['unit_usul']);echo '</pre>';die;
                            $subdata['subunit_usul']  		= $this->rsa_pup_model->get_pup_subunit_usul_verifikator($user->id,$tahun);
...
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
                ...
                function daftar_unit_kbuu($tahun=''){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==11))){
                            $data['main_menu']              = $this->load->view('main_menu','',TRUE);
                            $subdata['unit_usul'] 		= $this->rsa_pup_model->get_pup_unit_usul($tahun);
                            // echo '<pre>';var_dump($subdata['unit_usul']);echo '</pre>';die;
                            $subdata['subunit_usul'] 		= $this->rsa_pup_model->get_pup_subunit_usul($tahun);
                            $subdata['cur_tahun'] =  $tahun;
                            $subdata['jenis'] =  'pup';
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
    ...
    //			echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function daftar_spm_kbuu($tahun="",$kode_unit_subunit=""){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==11))){
                            ...
                            $subdata['daftar_spm']          = $this->rsa_pup_model->get_daftar_spp($kode_unit_subunit,$tahun);
//                            echo '<pre>';var_dump($subdata['daftar_spp']);echo '</pre>';die;
    ...
    //          echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function spm_pup_verifikator($url = ''){
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE); 
                                ...
                                $dokumen =  'SPM-DRAFT-KPA' ; //$this->rsa_pup_model->check_dokumen_pup_by_str_trx($url);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['detail_verifikator']  = $this->rsa_pup_model->get_verifikator_by_spm($nomor_trx_spm);
                                ...
                                $subdata['ket'] = $this->rsa_pup_model->lihat_ket_by_str_trx($nomor_trx_spp);
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE); 
                                ...
                                // $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($url);

                              
                                $dokumen = 'SPM-FINAL-VERIFIKATOR';//$dokumen_pup;
...
//                        echo $h;
                        ...
                        $this->pdfgen->cetak($html,'SPP_pup_'.$unit.'_'.''.$tahun);
                    }
...
//                        echo $html;die;
                        $this->load->library('Pdfgen'); 
                        $this->pdfgen->cetak($html,'SPM_pup_');
                    }
                }
                
                function usulkan_spp_pup(){
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==13))){
                        if($this->input->post('proses')){
                            ...
                            $id_nomor_pup = $this->input->post('id_nomor_pup') ;
                            ...
                            $data = array(
                                ...
                                'bulan' => $bulan,
                                'tahun' => $tahun,
//                                'urut' => $this->rsa_pup_model->get_urut($kd_unit,$jenis,$tahun)
                            );
                            
                            $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($nomor_trx);

                            $check_exist_pup = $this->rsa_pup_model->check_exist_pup($nomor_trx,$this->cur_tahun);

                            if((!$check_exist_pup)&&($dokumen_pup == '')){

                                if($this->rsa_pup_model->proses_nomor_spp_pup($kd_unit,$data) ){
                                    
                                    $data_spp = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spp' => $this->rsa_pup_model->get_id_nomor_pup($jenis,$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spp' => $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx),
                                        'str_nomor_trx' => $nomor_trx,
                                    ...
                                    $data = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        'posisi' => $proses,
                                        // 'id_trx_nomor_pup' => $this->rsa_pup_model->get_id_nomor_pup($jenis,$kd_unit,$this->cur_tahun),
                                        'id_trx_nomor_pup' => $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx),
                                        'ket' => $ket,
                                        'aktif' => '1',
                                        'tahun' => $tahun,
                                        'tgl_proses' => date("Y-m-d H:i:s")
                                   ...
                                   // var_dump($data);die;
                                    ...
                                    if($this->rsa_pup_model->proses_pup($kd_unit,$data,$id_nomor_pup)&& $this->rsa_pup_model->proses_data_spp($data_spp)){
...
//                                'urut' => $this->rsa_pup_model->get_urut($kd_unit,$jenis,$tahun)
                            ...
                            $dokumen_pup = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($nomor_trx_spp);

                            $check_exist_pup = $this->rsa_pup_model->check_exist_pup($nomor_trx,$this->cur_tahun);
                            
                            
                            if((!$check_exist_pup)&&($dokumen_pup == 'SPP-FINAL')){

                                if($this->rsa_pup_model->proses_nomor_spm_pup($kd_unit,$data)){

                                    $id_trx_nomor_pup = $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx_spp) ;
                                    $id_trx_nomor_pup_spm = $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx) ;

                                    $data_spm = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spm' => $this->rsa_pup_model->get_id_nomor_pup($jenis,$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spm' => $id_trx_nomor_pup_spm,
                                        'str_nomor_trx' => $nomor_trx,
                                        'jumlah_bayar' => $this->input->post('jumlah_bayar'),
                                        'terbilang' => $this->input->post('terbilang'),
                                        'untuk_bayar' => $this->input->post('untuk_bayar'),
                                    ...
                                    $data_spp_spm = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spp' => $this->rsa_pup_model->get_id_nomor_pup('SPP',$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spp' => $id_trx_nomor_pup,
                                        'str_nomor_trx_spp' => $nomor_trx_spp,
                                        // 'nomor_trx_spm' => $this->rsa_pup_model->get_id_nomor_pup('SPM',$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spm' => $id_trx_nomor_pup_spm,
                                    ...
                                    $data = array(
                                        ...
                                        'posisi' => $proses,
                                        // 'id_trx_nomor_pup' => $this->rsa_pup_model->get_id_nomor_pup($jenis,$kd_unit,$this->cur_tahun),
                                        'id_trx_nomor_pup' => $id_trx_nomor_pup,
                                        'id_trx_nomor_pup_spm' => $id_trx_nomor_pup_spm,
                                        'ket' => $ket,
                                        'aktif' => '1',
                                        'tahun' => $tahun,
                                        'tgl_proses' => date("Y-m-d H:i:s")
                                   ...
                                   // var_dump($data);die;
                                    ...
                                    if($this->rsa_pup_model->proses_pup($kd_unit,$data,$id_trx_nomor_pup) && $this->rsa_pup_model->proses_trx_spp_spm($data_spp_spm) && $this->rsa_pup_model->proses_data_spm($data_spm)){
                                        $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                        echo "sukses";
                                    }else{
                                        echo "gagal";
                ...
                function proses_spp_pup(){
                    if($this->input->post('proses')){
                        ...
                        $dokumen = $this->rsa_pup_model->check_dokumen_pup_by_str_trx($nomor_trx);
                        ...
                        if($ok){
                            ...
                            if($this->rsa_pup_model->proses_pup($kd_unit,$data,$id_nomor_pup)){
                                $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                echo "sukses";
                            }else{
                                echo "gagal";
                ...
                function proses_spm_pup(){
                    if($this->input->post('proses')){
                        ...
                        $dokumen = $this->rsa_pup_model->check_dokumen_pup($kd_unit,$tahun,$id_nomor_pup);
                        ...
                        if($ok){
                            ...
                            $nomor_trx_spm = $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx) ;
                            ...
                            if($this->rsa_pup_model->proses_pup($kd_unit,$data,$id_nomor_pup)){
                                if($this->check_session->get_level() == 3){
                                    ...
                                    $data_verifikator = array(
                                        'nomor_trx_spm' => $nomor_trx_spm ,//$this->rsa_pup_model->get_id_nomor_pup('SPM',$kd_unit,$tahun),//$nomor_[0],
                                        'str_nomor_trx_spm' => $nomor_trx,
                                        'kode_unit_subunit' => $kd_unit,
                                        'jenis_trx' => 'PUP',
                                    ...
                                    if($this->rsa_pup_model->proses_verifikator_pup($data_verifikator)){
                                        $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                        echo "sukses";
                                    }else{
                                        echo "gagal";
                ...
                function proses_final_pup(){
                    if($this->input->post('proses')){
                        ...
                        $id_trx_nomor_pup = $this->input->post('id_nomor_pup') ;
                        $id_trx_nomor_pup_spm = $this->rsa_pup_model->get_id_nomor_pup_by_nomor_trx($nomor_trx_spm) ;

                        $nomor_trx_spp = $this->rsa_pup_model->get_spp_by_spm($nomor_trx_spm);
                        ...
                        if(($proses == 'SPM-FINAL-KBUU')&&($dokumen_pup == 'SPM-FINAL-VERIFIKATOR')){
                            $ok = TRUE ;
                        }else{
                            $ok = FALSE;
                        }
...
//                            echo $saldo_ben->saldo;die;
                            ...
                            // echo $kd_unit . ' ' . $id_trx_nomor_pup ; die;
                            
                            if($this->rsa_pup_model->proses_pup($kd_unit,$data,$id_trx_nomor_pup)){
                                ...
                                $data_spm_cair = array(
                                    'no_urut' => $this->rsa_pup_model->get_next_urut_spm_cair($this->input->post('tahun')),
//                                    'nomor_trx_spm' => $this->rsa_pup_model->get_id_nomor_pup('SPM',$kd_unit,$this->input->post('tahun')),//,$nomor,
    ...
    //                            var_dump($data_spm_cair);die;
                                ...
                                // $data_to_nihil = array(
                                //     'kode_unit_subunit' => $kd_unit,
                                //     'str_nomor_trx_spm_pup' => $nomor_trx_spm,
                                //     'tgl_proses_pup' => date('Y-m-d H:i:s'),
                                //     'tahun' => $this->input->post('tahun'),
                                //     'status' => '1',
                                // );

                                if($this->rsa_pup_model->final_pup($kd_unit,$data) && $this->kas_undip_model->isi_trx($data_kas) && $this->rsa_pup_model->spm_cair($data_spm_cair)){ //&& $this->rsa_pup_model->ks_to_nihil($data_pup_to_nihil)){

                                // if($this->kas_undip_model->isi_trx($data_kas) && $this->rsa_pup_model->spm_cair($data_spm_cair)){
				...
				if($this->check_session->user_session() && $this->check_session->get_level()==2){
						...
						// $kode = strlen(($top_unit->kode_unit+1)+"")==1?"0".($top_unit->kode_unit+1):$top_unit->kode_unit+1;
                                        if($this->input->post()){
                                            ...
                                            $this->form_validation->set_rules('no_pup','no_pup','required|min_length[2]|callback_check_exist_rsa_pup');
                                            ...
                                            if ($this->form_validation->run()){
												...
												'no_pup' => form_prep($this->input->post('no_pup')),
							                    ...
							                    $this->rsa_pup_model->add_rsa_pup($data);
                                                echo "berhasil";
		...
		if($this->check_session->user_session() && $this->check_session->get_level()==100){
			if($this->uri->segment(3)){
				if($this->rsa_pup_model->delete_rsa_pup(array('no'=>$this->uri->segment(3)))){
					$data['class']	 = 'option box';
					$data['class_btn']	 = 'ya';
		...
		function get_row_rsa_pup(){
			if($this->check_session->user_session() && $this->check_session->get_level()==100){
				$data['result_rsa_pup'] = $this->rsa_pup_model->get_rsa_pup();
				$this->load->view('rsa_permintaan/row_rsa_pup',$data);
			}
			else{
				show_404('page');
			}
		...
		function filter_rsa_pup(){
			if($this->check_session->user_session() && $this->check_session->get_level()==100){
				$keyword = form_prep($this->input->post('keyword'));
				$data['result_rsa_pup'] = $this->rsa_pup_model->search_rsa_pup($keyword);
				$this->load->view('rsa_permintaan/row_rsa_pup',$data);
			}
			else{
				show_404('page');
               ...
               function check_exist_rsa_pup($no_pup){

                        $this->form_validation->set_message('check_exist_rsa_pup', 'Maaf, kode RSA tsb sudah terdaftar');

                        $result = $this->rsa_pup_model->get_rsa_pup($no_pup);
                ...
                function check_exist_rba_unit($kode_unit_rba){
                        ...
                        $result = $this->rsa_pup_model->get_rsa_pup($kode_unit_rba);
                ...
                function saldo($tahun=''){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==3)||($this->check_session->get_level()==11))){
                            $data['main_menu']              = $this->load->view('main_menu','',TRUE);
                            $subdata['unit_usul'] 		= $this->rsa_pup_model->get_pup_unit($tahun);
                            $subdata['cur_tahun'] =  $tahun;
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
    ...
    //			echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function daftar_spp($tahun="",$kode_unit_subunit=""){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==13))){
                            ...
                            $subdata['daftar_spp']          = $this->rsa_pup_model->get_daftar_spp($kode_unit_subunit,$tahun);
    ...
    //          echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function get_status_nihil(){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==13))){
                           // echo '5' ; 
                        $status = $this->rsa_pup_model->get_status_nihil($this->check_session->get_unit(),$this->cur_tahun);
                ...
                function get_notif_approve(){
                    ...
                    if($this->check_session->user_session()){
                        ...
                        $notif = $this->rsa_pup_model->get_notif_approve($kode_unit_subunit,$level,$user->id);
                ...
                function get_data_untuk_pekerjaan(){
                    ...
                    $kode_unit_subunit = $this->check_session->get_unit();
                    $data = $this->rsa_pup_model->get_data_untuk_pekerjaan($d['untuk_bayar'],$kode_unit_subunit,$this->cur_tahun);
                    $json = [];
                ...
                function get_data_penerima(){
                    $q = $this->input->get('query');
                    $kode_unit_subunit = $this->check_session->get_unit();
                    $data = $this->rsa_pup_model->get_data_penerima($q,$kode_unit_subunit,$this->cur_tahun);
                    $json = [];
                ...
                function get_notif_approve_all(){
                    ...
                    if($this->check_session->user_session()){
                        ...
                        $notif_pup = $this->rsa_pup_model->get_notif_approve($kode_unit_subunit,$level,$user->id);
