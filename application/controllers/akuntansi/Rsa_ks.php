...
/* -------------- Constructor ------------- */
	...
	public function __construct(){ 
		parent::__construct();
			...
			$this->load->model(array('rsa_ks_model','akun_model','kas_undip_model','kas_bendahara_model'));//,'setting_ks_model'));
...
//                        $data['message']	= validation_errors();
		...
		function input_rsa_ks(){
			if($this->check_session->user_session() && $this->check_session->get_level()==13){
				...
				$data['main_menu']	= $this->load->view('main_menu','',TRUE);		
				//$subdata_rsa_ks['result_rsa_ks'] 		= $this->rsa_ks_model->search_rsa_ks();
				//$subdata['row_rsa_ks'] 			= $this->load->view("rsa_permintaan/row_rsa_ks",$subdata_rsa_ks,TRUE);
				$subdata['opt_unit_kepeg']		= $this->option->opt_unit_kepeg();
				$data['main_content'] 			= $this->load->view("rsa_permintaan/input_rsa_ks",$subdata,TRUE);
				$this->load->view('main_template',$data);
			}
			else{
                ...
                $id_max_nomor = $this->rsa_ks_model->get_nomor_spp_urut($this->check_session->get_unit(),$this->cur_tahun) ;

                $id_max_nomor = $id_max_nomor + 1 ;

                // echo  $id_max_nomor_ks; die;
                                
                                $dokumen = '';//$this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun,$id_max_nomor_ks);
...
//                                        'tgl_spp' => $rsa_user->tgl_spp,
                                    ...
                                    $nomor_trx_ = $this->rsa_ks_model->get_nomor_next_spp($this->check_session->get_unit(),$this->cur_tahun);
                                    setlocale(LC_ALL, 'id_ID.utf8');$bln = strftime("%h");$thn = strftime("%Y");  
                                    $nomor_trx = $nomor_trx_.'/'.$this->check_session->get_alias().'/'.'SPP-KS'.'/'.strtoupper($bln).'/'.$thn;
                ...
                $id_max_nomor_ks = $this->rsa_ks_model->get_nomor_spp_urut($this->check_session->get_unit(),$this->cur_tahun) ;

                $id_max_nomor_ks = $id_max_nomor_ks + 1 ;

                // echo  $id_max_nomor_ks; die;
                                
                                $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun,$id_max_nomor_ks);
                               ...
                               // echo $dokumen_ks ; die ;
                                ...
                                if(($dokumen_ks == '')||($dokumen_ks == 'SPP-DITOLAK')||($dokumen_ks == 'SPM-DITOLAK-KPA')||($dokumen_ks == 'SPM-DITOLAK-VERIFIKATOR')||($dokumen_ks == 'SPM-DITOLAK-KBUU')){
                                    $subdata['detail_ks']    = array(
                                                                    'nom' => '0',
                                                                    'terbilang' => $this->convertion->terbilang('0'), 
...
//                                    var_dump($subdata['detail_pic']);die;
    ...
    //                                if(($dokumen_ks == '') || ($dokumen_ks == 'SPP-DITOLAK') || ($dokumen_ks == 'SPM-DITOLAK')){
    //                                    $subdata['siap_spp'] = 'ok';
    //                                }else{
    //                                    $subdata['siap_spp'] = 'no_ok';
    //                                }

                                    $subdata['tgl_spp'] = $this->rsa_ks_model->get_tgl_spp($this->check_session->get_unit(),$this->cur_tahun);
                                    ...
                                    $nomor_trx = $this->rsa_ks_model->get_nomor_spp($this->check_session->get_unit(),$this->cur_tahun); 
                                    $data_spp = $this->rsa_ks_model->get_data_spp($nomor_trx);
                                    $subdata['detail_ks']    = array(
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
    ...
    //                                }
                                ...
                                $subdata['id_nomor_ks'] = $id_max_nomor_ks ;
                                
                                $subdata['doc_ks'] = $dokumen_ks;
                                $subdata['nomor_spp'] = $nomor_trx;
                                $subdata['ket'] = $this->rsa_ks_model->lihat_ket($this->check_session->get_unit(),$this->cur_tahun);
                                
//                                $urut_spp_pup = $this->rsa_ks_model->get_urut($this->check_session->get_unit(),'SPP',$this->cur_tahun);
                                
                //$subdata['opt_unit_kepeg']        = $this->option->opt_unit_kepeg();
//                                var_dump($subdata['detail_pic']);die;
                $data['main_content']           = $this->load->view("rsa_permintaan/spumk_ks",$subdata,TRUE);
                $this->load->view('main_template',$data);
            } 
            else{
                redirect('welcome','refresh');  // redirect ke halaman home
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE);       
                                ...
                                $dokumen = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($url);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['id_nomor_ks'] = $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx);

                                $subdata['doc'] = $dokumen;
                                $subdata['nomor_spp'] = $nomor_trx;
                                $subdata['detail_ppk']  = $this->user_model->get_detail_rsa_user($kd_unit,'14');

                                $subdata['ket'] = $this->rsa_ks_model->lihat_ket_by_nomor_trx($nomor_trx);
                ...
                function spm_ks_lihat_99($url,$kd_unit,$tahun,$id_kuitansi = null){
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE);       
                                ...
                                $id_nomor = $this->rsa_ks_model->get_nomor_spp_urut_by_nomor_spp($nomor_trx_spp) ;
                                
                                
                                $dokumen = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($url);

                                $nomor_trx_spm = $this->rsa_ks_model->get_spm_by_spp($nomor_trx_spp);
                                ...
                                // SPP //
                                    ...
                                    $data_spp = $this->rsa_ks_model->get_data_spp($nomor_trx_spp);
...
//                                    echo $data_spp->tgl_spp; die
                                ...
                                // SPM //
                                    ...
                                    // $nomor_trx_spm = $this->rsa_ks_model->get_nomor_spm($this->check_session->get_unit(),$this->cur_tahun);  

                                     // $id_nomor_ks_spm = $this->rsa_ks_model->get_nomor_spm_urut_by_nomor_spp($nomor_trx_spp) ;
                                    
                                    // $nomor_trx_spm = $this->rsa_ks_model->get_nomor_spm_by_id($id_nomor_ks_spm);  
                                    
                                    $data_spm = $this->rsa_ks_model->get_data_spm($nomor_trx_spm);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                // $subdata['tgl_spm_kpa'] = $this->rsa_ks_model->get_tgl_spm_kpa($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_kpa'] = $this->rsa_ks_model->get_tgl_spm_kpa_by_spp($nomor_trx_spp);
                                
                                // $subdata['tgl_spm_verifikator'] = $this->rsa_ks_model->get_tgl_spm_verifikator($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_verifikator'] = $this->rsa_ks_model->get_tgl_spm_verifikator_by_spp($nomor_trx_spp);
                                
                                // $subdata['tgl_spm_kbuu'] = $this->rsa_ks_model->get_tgl_spm_kbuu($kd_unit ,$this->cur_tahun,$nomor_trx_spm);

                                $subdata['tgl_spm_kbuu'] = $this->rsa_ks_model->get_tgl_spm_kbuu_by_spp($nomor_trx_spp);
                                
                                // $subdata['ket'] = $this->rsa_ks_model->lihat_ket($this->check_session->get_unit(),$this->cur_tahun);
                                // echo $this->rsa_ks_model->lihat_ket_by_nomor_trx($nomor_trx_spp); die ;
                                $subdata['ket'] = $this->rsa_ks_model->lihat_ket_by_nomor_trx($nomor_trx_spp);
...
//                                var_dump($subdata);die;
				...
				$data['main_menu']	= $this->load->view('main_menu','',TRUE);	
                                ...
                                $id_nomor = $this->rsa_ks_model->get_nomor_spp_urut_by_nomor_spp($str_nomor_trx_spp) ;
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                //////////
                                    ...
                                    $nomor_trx_ = $this->rsa_ks_model->get_nomor_next_spm($this->check_session->get_unit(),$this->cur_tahun);
...
//                                    $subdata['detail_pic']  = $this->user_model->get_detail_rsa_user($this->check_session->get_unit(),'13');
                                    ...
                                    $subdata['detail_verifikator']  = $this->rsa_ks_model->get_verifikator(substr($this->check_session->get_unit(),0,2));
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['tgl_spm_kpa'] = $this->rsa_ks_model->get_tgl_spm_kpa($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                                
                                $subdata['tgl_spm_verifikator'] = $this->rsa_ks_model->get_tgl_spm_verifikator($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                                
                                $subdata['tgl_spm_kbuu'] = $this->rsa_ks_model->get_tgl_spm_kbuu($this->check_session->get_unit(),$this->cur_tahun,$nomor_trx_spm);
                ...
                //$subdata_rsa_ks['result_rsa_ks']        = $this->rsa_ks_model->search_rsa_ks();
//              $subdata['detail_ks']            = array(
//                                                                                    'nom' => $this->setting_ks_model->get_setting_ks($this->check_session->get_unit(),$this->cur_tahun),
//                                                                                    'terbilang' => $this->convertion->terbilang($this->setting_ks_model->get_setting_ks($this->check_session->get_unit(),$this->cur_tahun)), 
//                                                                                    
//                                                                                );
                                ...
                                $dokumen = 'SPP-FINAL' ; // $this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_ks);
...
//                                $subdata['detail_pic']  = $this->user_model->get_detail_rsa_user($this->check_session->get_unit(),'13');
                                ...
                                // $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun);

                                
                                
//                                $subdata['tgl_spm'] = $this->rsa_ks_model->get_tgl_spm($this->check_session->get_unit(),$this->cur_tahun);
                                
                                // $nomor_trx_spp = $this->rsa_ks_model->get_nomor_spp($this->check_session->get_unit(),$this->cur_tahun); 
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                /////////
                                    ...
                                    $nomor_trx_ = $this->rsa_ks_model->get_nomor_next_spm($this->check_session->get_unit(),$this->cur_tahun);
                                    setlocale(LC_ALL, 'id_ID.utf8');$bln = strftime("%h");$thn = strftime("%Y");
                                    $nomor_trx_spm = $nomor_trx_.'/'.$this->check_session->get_alias().'/'.'SPM-KS'.'/'.strtoupper($bln).'/'.$thn;
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
				...
				//$subdata_rsa_ks['result_rsa_ks'] 		= $this->rsa_ks_model->search_rsa_ks();
                                ...
                                // echo  $id_nomor_ks; die;
                                
                                $dokumen = 'SPM-DRAFT-PPK';//this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_ks);
                                ...
                                // $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks($this->check_session->get_unit(),$this->cur_tahun,$id_nomor_ks);
                                
                                $nomor_trx_spp = $str_nomor_trx_spp ; // $this->rsa_ks_model->get_nomor_spp_by_id($id_nomor_ks); 
...
//                            $subdata['unit_usul'] 		= $this->rsa_ks_model->get_ks_unit_usul_verifikator($user->id,$tahun);
                            $subdata['unit_usul'] 		= $this->rsa_ks_model->get_ks_unit_usul_verifikator($user->id,$tahun);
                            // echo '<pre>';var_dump($subdata['unit_usul']);echo '</pre>';die;
                            $subdata['subunit_usul']  		= $this->rsa_ks_model->get_ks_subunit_usul_verifikator($user->id,$tahun);
...
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
                ...
                function daftar_unit_kbuu($tahun=''){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==11))){
                            $data['main_menu']              = $this->load->view('main_menu','',TRUE);
                            $subdata['unit_usul'] 		= $this->rsa_ks_model->get_ks_unit_usul($tahun);
                            // echo '<pre>';var_dump($subdata['unit_usul']);echo '</pre>';die;
                            $subdata['subunit_usul'] 		= $this->rsa_ks_model->get_ks_subunit_usul($tahun);
                            $subdata['cur_tahun'] =  $tahun;
                            $subdata['jenis'] =  'ks';
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
    ...
    //			echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function daftar_spm_kbuu($tahun="",$kode_unit_subunit=""){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==11))){
                            ...
                            $subdata['daftar_spm']          = $this->rsa_ks_model->get_daftar_spp($kode_unit_subunit,$tahun);
//                            echo '<pre>';var_dump($subdata['daftar_spp']);echo '</pre>';die;
    ...
    //          echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function spm_ks_verifikator($url = ''){
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE); 
                                ...
                                $dokumen =  'SPM-DRAFT-KPA' ; //$this->rsa_ks_model->check_dokumen_ks_by_str_trx($url);
...
//                                        'tgl_spp' => $data_spp->tgl_spp,
                                ...
                                $subdata['detail_verifikator']  = $this->rsa_ks_model->get_verifikator_by_spm($nomor_trx_spm);
                                ...
                                $subdata['ket'] = $this->rsa_ks_model->lihat_ket_by_str_trx($nomor_trx_spp);
                ...
                $data['main_menu']  = $this->load->view('main_menu','',TRUE); 
                                ...
                                // $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($url);

                              
                                $dokumen = 'SPM-FINAL-VERIFIKATOR';//$dokumen_ks;
...
//                        echo $h;
                        ...
                        $this->pdfgen->cetak($html,'SPP_ks_'.$unit.'_'.''.$tahun);
                    }
...
//                        echo $html;die;
                        $this->load->library('Pdfgen'); 
                        $this->pdfgen->cetak($html,'SPM_ks_');
                    }
                }
                
                function usulkan_spp_ks(){
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==13))){
                        if($this->input->post('proses')){
                            ...
                            $id_nomor_ks = $this->input->post('id_nomor_ks') ;
                            ...
                            $data = array(
                                ...
                                'bulan' => $bulan,
                                'tahun' => $tahun,
//                                'urut' => $this->rsa_ks_model->get_urut($kd_unit,$jenis,$tahun)
                            );
                            
                            $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($nomor_trx);

                            $check_exist_ks = $this->rsa_ks_model->check_exist_ks($nomor_trx,$this->cur_tahun);

                            if((!$check_exist_ks)&&($dokumen_ks == '')){

                                if($this->rsa_ks_model->proses_nomor_spp_ks($kd_unit,$data) ){
                                    
                                    $data_spp = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spp' => $this->rsa_ks_model->get_id_nomor_ks($jenis,$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spp' => $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx),
                                        'str_nomor_trx' => $nomor_trx,
                                    ...
                                    $data = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        'posisi' => $proses,
                                        // 'id_trx_nomor_ks' => $this->rsa_ks_model->get_id_nomor_ks($jenis,$kd_unit,$this->cur_tahun),
                                        'id_trx_nomor_ks' => $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx),
                                        'ket' => $ket,
                                        'aktif' => '1',
                                        'tahun' => $tahun,
                                        'tgl_proses' => date("Y-m-d H:i:s")
                                   ...
                                   // var_dump($data);die;
                                    ...
                                    if($this->rsa_ks_model->proses_ks($kd_unit,$data,$id_nomor_ks)&& $this->rsa_ks_model->proses_data_spp($data_spp)){
...
//                                'urut' => $this->rsa_ks_model->get_urut($kd_unit,$jenis,$tahun)
                            ...
                            $dokumen_ks = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($nomor_trx_spp);

                            $check_exist_ks = $this->rsa_ks_model->check_exist_ks($nomor_trx,$this->cur_tahun);
                            
                            if((!$check_exist_ks)&&($dokumen_ks == 'SPP-FINAL')){

                            

                                if($this->rsa_ks_model->proses_nomor_spm_ks($kd_unit,$data)){

                                    $id_trx_nomor_ks = $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx_spp) ;
                                    $id_trx_nomor_ks_spm = $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx) ;

                                    $data_spm = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spm' => $this->rsa_ks_model->get_id_nomor_ks($jenis,$kd_unit,$this->cur_tahun),
                                    ...
                                    $data_spp_spm = array(
                                        'kode_unit_subunit' => $kd_unit,
                                        // 'nomor_trx_spp' => $this->rsa_ks_model->get_id_nomor_ks('SPP',$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spp' => $id_trx_nomor_ks,
                                        'str_nomor_trx_spp' => $nomor_trx_spp,
                                        // 'nomor_trx_spm' => $this->rsa_ks_model->get_id_nomor_ks('SPM',$kd_unit,$this->cur_tahun),
                                        'nomor_trx_spm' => $id_trx_nomor_ks_spm,
                                    ...
                                    $data = array(
                                        ...
                                        'posisi' => $proses,
                                        // 'id_trx_nomor_ks' => $this->rsa_ks_model->get_id_nomor_ks($jenis,$kd_unit,$this->cur_tahun),
                                        'id_trx_nomor_ks' => $id_trx_nomor_ks,
                                   ...
                                   // var_dump($data);die;
                                    ...
                                    if($this->rsa_ks_model->proses_ks($kd_unit,$data,$id_trx_nomor_ks) && $this->rsa_ks_model->proses_trx_spp_spm($data_spp_spm) && $this->rsa_ks_model->proses_data_spm($data_spm)){
                                        $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                        echo "sukses";
                                    }else{
                                        echo "gagal";
                ...
                function proses_spp_ks(){
                    if($this->input->post('proses')){
                        ...
                        $dokumen = $this->rsa_ks_model->check_dokumen_ks_by_str_trx($nomor_trx);
                        ...
                        if($ok){
                            ...
                            if($this->rsa_ks_model->proses_ks($kd_unit,$data,$id_nomor_ks)){
                                $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                echo "sukses";
                            }else{
                                echo "gagal";
                ...
                function proses_spm_ks(){
                    if($this->input->post('proses')){
                        ...
                        $dokumen = $this->rsa_ks_model->check_dokumen_ks($kd_unit,$tahun,$id_nomor_ks);
                        ...
                        if($ok){
                            ...
                            $nomor_trx_spm = $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx) ;
                            ...
                            if($this->rsa_ks_model->proses_ks($kd_unit,$data,$id_nomor_ks)){
                                if($this->check_session->get_level() == 3){
                                    ...
                                    $data_verifikator = array(
                                        'nomor_trx_spm' => $nomor_trx_spm ,//$this->rsa_ks_model->get_id_nomor_ks('SPM',$kd_unit,$tahun),//$nomor_[0],
                                        'str_nomor_trx_spm' => $nomor_trx,
                                        'kode_unit_subunit' => $kd_unit,
                                        'jenis_trx' => 'KS',
                                    ...
                                    if($this->rsa_ks_model->proses_verifikator_ks($data_verifikator)){
                                        $this->session->set_flashdata('message', '<div class="alert alert-success" style="text-align:center"><span class="glyphicon glyphicon glyphicon-ok" aria-hidden="true"></span> SPP/SPM anda berhasil disubmit.</div>');
                                        echo "sukses";
                                    }else{
                                        echo "gagal";
                ...
                function proses_final_ks(){
                    if($this->input->post('proses')){
                        ...
                        $id_trx_nomor_ks = $this->input->post('id_nomor_ks') ;
                        $id_trx_nomor_ks_spm = $this->rsa_ks_model->get_id_nomor_ks_by_nomor_trx($nomor_trx_spm) ;

                        $nomor_trx_spp = $this->rsa_ks_model->get_spp_by_spm($nomor_trx_spm);
                        ...
                        if(($proses == 'SPM-FINAL-KBUU')&&($dokumen_ks == 'SPM-FINAL-VERIFIKATOR')){
                            $ok = TRUE ;
                        }else{
                            $ok = FALSE;
                        }
...
//                            echo $saldo_ben->saldo;die;
                            ...
                            // echo $kd_unit . ' ' . $id_trx_nomor_ks ; die;
                            
                            if($this->rsa_ks_model->proses_ks($kd_unit,$data,$id_trx_nomor_ks)){
                                ...
                                // $saldo_penerima = $this->kas_bendahara_model->get_kas_bendahara_by_kd_akun_and_unit($kd_akun_kas_bendahara,$kd_unit,'KS',$this->cur_tahun);

                                // echo $saldo_penerima; die;
                                
                                $debet = $this->input->post('kredit') ;
                                // $saldo_pic_akhir =  $saldo_penerima + $debet ;
                                $saldo_pic_akhir =  $this->input->post('kredit') ;
                                ...
                                $data_spm_cair = array(
                                    'no_urut' => $this->rsa_ks_model->get_next_urut_spm_cair($this->input->post('tahun')),
//                                    'nomor_trx_spm' => $this->rsa_ks_model->get_id_nomor_ks('SPM',$kd_unit,$this->input->post('tahun')),//,$nomor,
    ...
    //                            var_dump($data_spm_cair);die;
                                ...
                                //     'kode_unit_subunit' => $kd_unit,
                                //     'str_nomor_trx_spm_ks' => $nomor_trx_spm,
                                //     'tgl_proses_ks' => $tgl_proses,
                                //     'tahun' => $this->input->post('tahun'),
                                //     'aktif' => '1',
                                // );

                                if($this->rsa_ks_model->final_ks($kd_unit,$data) && $this->kas_undip_model->isi_trx($data_kas) && $this->rsa_ks_model->spm_cair($data_spm_cair)){// && $this->rsa_ks_model->data_ks_to_nihil($data_ks_to_nihil)){

                                // if($this->kas_undip_model->isi_trx($data_kas) && $this->rsa_ks_model->spm_cair($data_spm_cair)){
				...
				if($this->check_session->user_session() && $this->check_session->get_level()==2){
						...
						// $kode = strlen(($top_unit->kode_unit+1)+"")==1?"0".($top_unit->kode_unit+1):$top_unit->kode_unit+1;
                                        if($this->input->post()){
                                            ...
                                            $this->form_validation->set_rules('no_ks','no_ks','required|min_length[2]|callback_check_exist_rsa_ks');
                                            ...
                                            if ($this->form_validation->run()){
												...
												'no_ks' => form_prep($this->input->post('no_ks')),
							                    ...
							                    $this->rsa_ks_model->add_rsa_ks($data);
                                                echo "berhasil";
		...
		if($this->check_session->user_session() && $this->check_session->get_level()==100){
			if($this->uri->segment(3)){
				if($this->rsa_ks_model->delete_rsa_ks(array('no'=>$this->uri->segment(3)))){
					$data['class']	 = 'option box';
					$data['class_btn']	 = 'ya';
		...
		function get_row_rsa_ks(){
			if($this->check_session->user_session() && $this->check_session->get_level()==100){
				$data['result_rsa_ks'] = $this->rsa_ks_model->get_rsa_ks();
				$this->load->view('rsa_permintaan/row_rsa_ks',$data);
			}
			else{
				show_404('page');
			}
		...
		function filter_rsa_ks(){
			if($this->check_session->user_session() && $this->check_session->get_level()==100){
				$keyword = form_prep($this->input->post('keyword'));
				$data['result_rsa_ks'] = $this->rsa_ks_model->search_rsa_ks($keyword);
				$this->load->view('rsa_permintaan/row_rsa_ks',$data);
			}
			else{
				show_404('page');
               ...
               function check_exist_rsa_ks($no_ks){

                        $this->form_validation->set_message('check_exist_rsa_ks', 'Maaf, kode RSA tsb sudah terdaftar');

                        $result = $this->rsa_ks_model->get_rsa_ks($no_ks);
                ...
                function check_exist_rba_unit($kode_unit_rba){
                        ...
                        $result = $this->rsa_ks_model->get_rsa_ks($kode_unit_rba);
                ...
                function saldo($tahun=''){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==3)||($this->check_session->get_level()==11))){
                            $data['main_menu']              = $this->load->view('main_menu','',TRUE);
                            $subdata['unit_usul'] 		= $this->rsa_ks_model->get_ks_unit($tahun);
                            $subdata['cur_tahun'] =  $tahun;
//                            $subdata['opt_sumber_dana'] 	= $this->option->sumber_dana();
    ...
    //          echo '<pre>';var_dump($subdata['unit_usul_impor']);echo '</pre>';die;
                ...
                function get_status_nihil(){
                    ...
                    if($this->check_session->user_session() && (($this->check_session->get_level()==100)||($this->check_session->get_level()==13))){
                           // echo '5' ; 
                        $status = $this->rsa_ks_model->get_status_nihil($this->check_session->get_unit(),$this->cur_tahun);
                ...
                function get_notif_approve(){
                    ...
                    if($this->check_session->user_session()){
                        ...
                        $notif = $this->rsa_ks_model->get_notif_approve($kode_unit_subunit,$level,$user->id);
                ...
                function get_data_untuk_pekerjaan(){
                    ...
                    $kode_unit_subunit = $this->check_session->get_unit();
                    $data = $this->rsa_ks_model->get_data_untuk_pekerjaan($d['untuk_bayar'],$kode_unit_subunit,$this->cur_tahun);
                    $json = [];
                ...
                function get_data_penerima(){
                    $q = $this->input->get('query');
                    $kode_unit_subunit = $this->check_session->get_unit();
                    $data = $this->rsa_ks_model->get_data_penerima($q,$kode_unit_subunit,$this->cur_tahun);
                    $json = [];
                ...
                function get_notif_approve_all(){
                    ...
                    if($this->check_session->user_session()){
                        ...
                        $notif_ks = $this->rsa_ks_model->get_notif_approve($kode_unit_subunit,$level,$user->id);
