                ...
                function create_spp_tup(){
                    if($this->check_session->user_session() && (($this->check_session->get_level()==13)||($this->check_session->get_level()==100))){
                        if($this->input->post('rel_kuitansi')){
                            ...
                            // $cookie = array(
                            //           'name'   => 'array_id_pengembalian',
                            //           'value'  => $data_pengembalian,
                            //           );
...
//                            $data = $this->encrypt->encode($array_id);
                            ...
                            redirect(site_url('rsa_tup/spp_tup/')) ;
                        }
                ...
                function get_last_spp(){
                    if($this->check_session->user_session() && (($this->check_session->get_level()==13)||($this->check_session->get_level()==100))){
                        $spp = $this->rsa_tup_model->get_last_spp($this->check_session->get_unit(),$this->cur_tahun);
                        // var_dump($spp); die;
                        if(!empty($spp)){
                            redirect(site_url('rsa_tup/spp_tup_lihat/' .urlencode(base64_encode($spp->str_nomor_trx))));
                        }else{
                             redirect(site_url('rsa_tup/daftar_spp/'));
                        }
                    }
                }
...
//                                    echo '<pre>';print_r($rincian_akun_pengeluaran);echo '</pre>';die;
                                ...
                                $subdata['ket'] = $this->rsa_tup_model->lihat_ket_by_str_trx($nomor_trx);//$kd_unit,$tahun);

                $data['main_content']           = $this->load->view("rsa_tup/spp_tup_lihat",$subdata,TRUE);
                $this->load->view('main_template',$data);
            } 
            else{
                redirect('welcome','refresh');  // redirect ke halaman home
...
//                                var_dump($subdata);die;
                ...
                function spm_tup_lihat_99($url,$kd_unit,$tahun){
...
//                                var_dump($subdata);die;
                ...
                function spm_tup_kbuu($kd_unit,$tahun){
...
//                                var_dump( $subdata['kas_undip']);die;
                ...
                //$subdata['opt_unit_kepeg']        = $this->option->opt_unit_kepeg();
//                                var_dump($subdata);die;
                $data['main_content']           = $this->load->view("rsa_tup/spm_tup_kbuu",$subdata,TRUE);
                $this->load->view('main_template',$data);
            } 
            else{
                redirect('welcome','refresh');  // redirect ke halaman home
...
//                            echo $saldo_ben->saldo;die;
                            ...
                            if($this->rsa_tup_model->proses_tup($kd_unit,$data)){
                                ...
                                $data_kredit = array(
                                    ...
                                    'kredit' => $this->input->post('kredit') + $pengembalian,
                                    'debet' => '0',
                                ...
                                if($kd_unit!='63'){
                                    ...
                                    $data_kas = array(
                                        ...
                                        'kd_akun_kas' => $kd_akun_kas,
                                        'kd_unit' => $kd_unit,//'99',
                                        'deskripsi' => 'PENGEMBALIAN TUP ( TUP-NIHIL ) UNIT ' . $kd_unit . '( NON CASH )',
                                        'no_spm' => $this->input->post('nomor_trx'),
                                        'debet' => $this->input->post('kredit') + $pengembalian ,
                                        'kredit' => '0',
                                        'saldo' => $saldo + ( $pengembalian + $this->input->post('kredit') ),
                                        'aktif' => '1',
                                        'tahun' => $this->input->post('tahun'),
                                    );
                                }
