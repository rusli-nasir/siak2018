...
//         }
        ...
        function get_daftar_spp($kode_unit_subunit,$tahun){
            ...
            $query = "SELECT tt1.str_nomor_trx AS str_nomor_trx_spp,
                        ...
                        FROM trx_nomor_lsk AS tt1
                        JOIN trx_lsk AS t1
                            ON t1.id_trx_nomor_lsk = tt1.id_trx_nomor_lsk
                        JOIN trx_spp_lsk_data AS t2
                            ON t2.nomor_trx_spp = tt1.id_trx_nomor_lsk
                        LEFT JOIN trx_spm_lsk_data AS t3
                            ON t3.nomor_trx_spm = t1.id_trx_nomor_lsk_spm
                        WHERE tt1.kode_unit_subunit = '{$kode_unit_subunit}'
                            AND jenis = 'SPP' 
                            AND tt1.tahun = '{$tahun}' 
                            AND t1.aktif = '1' 
                            AND t1.tgl_proses
                        GROUP BY tt1.id_trx_nomor_lsk
                        ORDER BY tt1.id_trx_nomor_lsk DESC" ;
    ...
    //                $this->db->update('rsa_kuitansi', array('str_nomor_trx'=>$data['str_nomor_trx']));
                    // $nw = date('Y-m-d H:i:s');
                        $query = "UPDATE rsa_detail_belanja_ "
                            ...
                            . "AND rsa_kuitansi_detail.kode_akun_tambah = rsa_detail_belanja_.kode_akun_tambah "
                            . "JOIN rsa_kuitansi ON rsa_kuitansi.id_kuitansi = rsa_kuitansi_detail.id_kuitansi "
                            . "SET rsa_detail_belanja_.proses = '64', rsa_detail_belanja_.tanggal_transaksi = '{$data['tgl_proses']}' "
                            . "WHERE rsa_kuitansi.str_nomor_trx_spm = '{$data['str_nomor_trx_spm']}'" ; 
    //                echo $query ; die;
                    $this->db->query($query);
                // }
