...
/* -------------- Constructor ------------- */
        ...
        function check_dokumen_ks($kode_unit_subunit,$tahun,$id_trx_nomor_ks = ""){
            $q = $this->db->query("SELECT posisi FROM trx_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND aktif = '1'  AND tahun = '{$tahun}' AND id_trx_nomor_ks = '{$id_trx_nomor_ks}' ");
//            var_dump($q->num_rows());die;
        ...
        function lihat_ket($kode_unit_subunit,$tahun){
            $q = $this->db->query("SELECT ket FROM trx_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND aktif = '1'  AND tahun = '{$tahun}' ");
//            var_dump($q->num_rows());die;
            if($q->num_rows() > 0){
               return $q->row()->ket ;
            }else{
        ...
        function lihat_ket_by_nomor_trx($str_nomor_trx){
            ...
            // echo"SELECT ket FROM trx_ks JOIN trx_nomor_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks WHERE trx_nomor_ks.str_nomor_trx = '{$str_nomor_trx}'" ; die ;

            $query = "SELECT trx_ks.ket FROM trx_ks JOIN trx_nomor_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks WHERE trx_nomor_ks.str_nomor_trx = '{$str_nomor_trx}' AND  trx_ks.aktif = '1' " ;


            // $querym = "SELECT * FROM trx_nomor_ks WHERE str_nomor_trx LIKE '{$str_nomor_trx}'" ;
        ...
        function proses_nomor_spp_ks($kode_unit,$data){
            
            // $this->db->where('tahun', $data['tahun']);
            // $this->db->where('aktif', '1');
            // $this->db->where('jenis', 'SPP');
            // $this->db->where('kode_unit_subunit', $kode_unit);
            // $this->db->update('trx_nomor_ks', array('aktif'=>'0')); 
            
            return $this->db->insert('trx_nomor_ks',$data);
            
        }
        
        function proses_nomor_spm_ks($kode_unit,$data){
        ...
        function proses_ks($kode_unit,$data,$id_nomor_ks){
            ...
            $this->db->where('aktif', '1');
            $this->db->where('kode_unit_subunit', $kode_unit);
            $this->db->where('id_trx_nomor_ks', $id_nomor_ks);
            $this->db->update('trx_ks', array('aktif'=>'0')); 
            
            return $this->db->insert('trx_ks',$data);
            
        }
        
        function final_ks($kode_unit,$data){
        ...
        function get_tgl_spp($kode_unit_subunit,$tahun){
            $q = $this->db->query("SELECT MAX(tgl_proses) AS tgl_proses FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif='1' ");
    //            var_dump($q->num_rows());die;
                if($q->num_rows() > 0){
...
//            echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' ";die;
            ...
            $q = $this->db->query("SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' ");
        ...
        function get_nomor_spp_by_id($id_trx_nomor_ks_spp){
            
//            echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' " ; die;

//            echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' ";die;

            $q = $this->db->query("SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE id_trx_nomor_ks = '{$id_trx_nomor_ks}' ");
...
//            echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' " ; die;
            ...
            $q = $this->db->query("SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPM'  AND tahun = '{$tahun}' AND aktif = '1' ");
        ...
        function get_nomor_spm_by_id($id_trx_nomor_ks_spm){
            
//            echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' AND aktif = '1' " ; die;
            

            $q = $this->db->query("SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE id_trx_nomor_ks = '{$id_trx_nomor_ks_spm}' ");

            // echo "SELECT str_nomor_trx AS nt FROM trx_nomor_ks WHERE id_trx_nomor_ks = '{$id_trx_nomor_ks}' " ;  die ;
        ...
        function get_nomor_next_spp($kode_unit_subunit,$tahun){
            $q = $this->db->query("SELECT IFNULL(MAX(nomor_trx),0) AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}' ");
//            var_dump($q->num_rows());die;
//            if($q->num_rows() > 0){
//               return $q->row()->tgl_proses ;
//            }else{
...
//            } 
        ...
        function get_nomor_next_spm($kode_unit_subunit,$tahun){
            $q = $this->db->query("SELECT IFNULL(MAX(nomor_trx),0) AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPM'  AND tahun = '{$tahun}' ");
//            var_dump($q->num_rows());die;
//            if($q->num_rows() > 0){
//               return $q->row()->tgl_proses ;
//            }else{
...
//            } 
        ...
        function get_id_nomor_ks($jenis,$kode_unit_subunit,$tahun){
//            echo "SELECT id_trx_nomor_ks FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}' AND tahun = '{$tahun}' AND aktif='1' "; die;
            $q = $this->db->query("SELECT id_trx_nomor_ks FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}' AND tahun = '{$tahun}' AND aktif='1' ");
//            var_dump($q->num_rows());die;
            if($q->num_rows() > 0){
               return $q->row()->id_trx_nomor_ks ;
            }else{
            
            }
        }

        function get_id_nomor_ks_by_nomor_trx($str_nomor_trx){
//            echo "SELECT id_trx_nomor_ks FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}' AND tahun = '{$tahun}' AND aktif='1' "; die;
            $q = $this->db->query("SELECT id_trx_nomor_ks FROM trx_nomor_ks WHERE str_nomor_trx = '{$str_nomor_trx}' ");
//            var_dump($q->num_rows());die;
            if($q->num_rows() > 0){
               return $q->row()->id_trx_nomor_ks ;
            }else{
        ...
        function get_tgl_spm($kode_unit_subunit,$tahun){
            $q = $this->db->query("SELECT tgl_proses AS tgl_proses FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPM'  AND tahun = '{$tahun}' AND aktif='1' ");
    //            var_dump($q->num_rows());die;
                if($q->num_rows() > 0){
                   return $q->row()->tgl_proses ;
                }else{
        ...
        function get_tgl_spm_kpa($kode_unit_subunit,$tahun,$nomor_trx_spm){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    . "JOIN trx_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks "
                    . "WHERE trx_nomor_ks.kode_unit_subunit = '{$kode_unit_subunit}' "
                    . "AND trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spm}' "
                    . "AND trx_ks.posisi = 'SPM-DRAFT-KPA' AND trx_nomor_ks.tahun = '{$tahun}' "
                    . "AND trx_nomor_ks.aktif = '1'";
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_tgl_spm_verifikator($kode_unit_subunit,$tahun,$nomor_trx_spm){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    ...
                    . "WHERE trx_nomor_ks.kode_unit_subunit = '{$kode_unit_subunit}' "
                    . "AND trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spm}' "
                    . "AND trx_ks.posisi = 'SPM-FINAL-VERIFIKATOR' AND trx_nomor_ks.tahun = '{$tahun}' "
                    . "AND trx_nomor_ks.aktif = '1'";
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_tgl_spm_kbuu($kode_unit_subunit,$tahun,$nomor_trx_spm){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    ...
                    . "WHERE trx_nomor_ks.kode_unit_subunit = '{$kode_unit_subunit}' "
                    . "AND trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spm}' "
                    . "AND trx_ks.posisi = 'SPM-FINAL-KBUU' AND trx_nomor_ks.tahun = '{$tahun}' "
                    . "AND trx_nomor_ks.aktif = '1'";
...
//            var_dump($str);die;
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_ks_unit_usul($tahun){
...
//                var_dump($result);die;
        ...
        function get_ks_subunit_usul($tahun){
...
//                var_dump($result);die;
        ...
        function get_ks_unit_usul_verifikator($id_user_verifikator,$tahun){
            
            // $query2 = "SELECT rba_2018.unit.nama_unit,rba_2018.unit.kode_unit,rsa_2018.trx_ks.posisi,rsa_2018.trx_ks.aktif,rsa_2018.trx_ks.tgl_proses,rsa_2018.trx_ks.tahun "
                    // . "FROM rba_2018.unit "
                    // . "JOIN rsa_verifikator_unit ON rba_2018.unit.kode_unit = rsa_verifikator_unit.kode_unit_subunit "
                    // . "LEFT JOIN rsa_2018.trx_ks ON rba_2018.unit.kode_unit = rsa_2018.trx_ks.kode_unit_subunit "
                    // . "WHERE rsa_verifikator_unit.id_user_verifikator = '{$id_user_verifikator}' "
                    // . "AND ( rsa_2018.trx_ks.aktif = '1' OR rsa_2018.trx_ks.aktif IS NULL ) "
                    // . "AND ( rsa_2018.trx_ks.tahun = '{$tahun}' OR rsa_2018.trx_ks.tahun IS NULL ) "
                    // . "GROUP BY rba_2018.unit.kode_unit "
                    // . "ORDER BY rba_2018.unit.kode_unit ASC";


            // $query = "SELECT rba_2018.unit.nama_unit,rba_2018.unit.kode_unit,rsa_2018.trx_ks.tahun,SUM(IF(rsa_2018.trx_ks.posisi='SPM-DRAFT-KPA',1,0)) AS posisi_ver,SUM(IF((rsa_2018.trx_ks.posisi='SPM-FINAL-VERIFIKATOR')OR(rsa_2018.trx_ks.posisi='SPM-FINAL-KBUU')OR(rsa_2018.trx_ks.posisi='SPM-FINAL-BUU'),1,0)) AS posisi_out_ver FROM rba_2018.unit JOIN rsa_verifikator_unit ON rba_2018.unit.kode_unit = rsa_verifikator_unit.kode_unit_subunit LEFT JOIN rsa_2018.trx_ks ON rba_2018.unit.kode_unit = rsa_2018.trx_ks.kode_unit_subunit WHERE rsa_verifikator_unit.id_user_verifikator = '{$id_user_verifikator}' AND ( rsa_2018.trx_ks.aktif = '1' OR rsa_2018.trx_ks.aktif IS NULL ) AND ( rsa_2018.trx_ks.tahun = '{$tahun}' OR rsa_2018.trx_ks.tahun IS NULL ) GROUP BY rba_2018.unit.kode_unit ORDER BY rba_2018.unit.kode_unit ASC" ;
...
//                var_dump($result);die;
        ...
        function get_ks_subunit_usul_verifikator($id_user_verifikator,$tahun){
            
            // $query = "SELECT rba_2018.subunit.nama_subunit,rba_2018.subunit.kode_subunit,rsa_2018.trx_ks.posisi,rsa_2018.trx_ks.aktif,rsa_2018.trx_ks.tgl_proses,rsa_2018.trx_ks.tahun "
            ...
            //         . "LEFT JOIN rsa_2018.trx_ks ON rba_2018.subunit.kode_subunit = rsa_2018.trx_ks.kode_unit_subunit "
            //         . "WHERE rsa_verifikator_unit.id_user_verifikator = '{$id_user_verifikator}' "
            //         . "AND ( rsa_2018.trx_ks.aktif = '1' OR rsa_2018.trx_ks.aktif IS NULL ) "
            //         . "AND ( rsa_2018.trx_ks.tahun = '{$tahun}' OR rsa_2018.trx_ks.tahun IS NULL ) "
            //         . "GROUP BY rba_2018.subunit.kode_subunit "
...
//                var_dump($q->row());die;
        ...
        function proses_verifikator_ks($data){
        //print_r($data);die;
...
//            }else{
        ...
        function get_daftar_spp($kode_unit_subunit,$tahun){
            ...
            $query = "SELECT tt1.str_nomor_trx AS str_nomor_trx_spp,t3.str_nomor_trx AS str_nomor_trx_spm,t1.tgl_proses,t2.untuk_bayar,t2.jumlah_bayar, t1.posisi FROM trx_nomor_ks AS tt1 JOIN trx_ks AS t1 ON t1.id_trx_nomor_ks = tt1.id_trx_nomor_ks JOIN trx_spp_ks_data AS t2 ON t2.nomor_trx_spp = tt1.id_trx_nomor_ks LEFT JOIN trx_spm_ks_data AS t3 ON t3.nomor_trx_spm = t1.id_trx_nomor_ks_spm WHERE tt1.kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP' AND tt1.tahun = '{$tahun}' AND t1.aktif = '1' GROUP BY tt1.id_trx_nomor_ks ORDER BY tt1.id_trx_nomor_ks DESC" ;
...
//            }
        ...
        function proses_data_spm($data){
            return $this->db->insert("trx_spm_ks_data",$data);
        }
        
        function get_data_spm($str_nomor_trx){
            $this->db->where('str_nomor_trx',$str_nomor_trx);
            $query = $this->db->get("trx_spm_ks_data");
    //  print_r($query);die;
//            if($query->num_rows()>0){
                    return $query->row();
//            }else{
...
//            $q = $this->db->query("SELECT IFNULL(MAX(urut),0) AS nt FROM trx_nomor_ks WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}'  AND tahun = '{$tahun}' ");
//            var_dump($q->num_rows());die;
//            if($q->num_rows() > 0){
//               return $q->row()->tgl_proses ;
//            }else{
...
//        }
            ...
            function ks_to_nihil($data_ks_to_nihil){
                ...
                $this->db->update('trx_ks_to_nihil', array('status'=>'0'));


                return $this->db->insert("trx_ks_to_nihil",$data_ks_to_nihil);

            }

            function data_ks_to_nihil($data_ks_to_nihil){

                $this->db->where('tahun', $data_ks_to_nihil['tahun']);
                $this->db->where('aktif', '1');
                $this->db->where('kode_unit_subunit', $data_ks_to_nihil['kode_unit_subunit']);
                $this->db->update('trx_ks_to_nihil', array('aktif'=>'0'));


                return $this->db->insert("trx_ks_to_nihil",$data_tup_to_nihil);
            ...
            function get_status_nihil($unit,$tahun){
                ...
                $query = "SELECT str_nomor_trx_spm_ks FROM trx_ks_to_nihil WHERE kode_unit_subunit = '{$unit}' AND status = '1'" ;
            ...
            function get_notif_approve($kode_unit_subunit,$level,$id_user_verifikator){
                    ...
                    if($level == '14'){ // PPK SUKPA
                           $query = "SELECT COUNT(posisi) AS jml FROM trx_ks WHERE ( posisi = 'SPP-DRAFT' OR posisi = 'SPP-FINAL' ) AND kode_unit_subunit = '{$kode_unit_subunit}' AND aktif = '1' " ; 

                        }else if($level == '2'){ // KPA
                            $query = "SELECT COUNT(posisi) AS jml FROM trx_ks WHERE posisi = 'SPM-DRAFT-PPK' AND kode_unit_subunit = '{$kode_unit_subunit}' AND aktif = '1' " ; 
                            ...
                            $query = "SELECT COUNT(posisi) AS jml FROM trx_ks WHERE posisi = 'SPM-DRAFT-KPA' AND SUBSTR(kode_unit_subunit,1,2) IN ({$str_unit}) AND aktif = '1' " ; 

                           // echo $query ; die ;

                        }else if($level == '11'){ // KBUU
                            $query = "SELECT COUNT(posisi) AS jml FROM trx_ks WHERE posisi = 'SPM-FINAL-VERIFIKATOR' AND aktif = '1' GROUP BY  posisi " ;
        ...
        function get_nomor_spp_urut($unit,$tahun){
            ...
            $query = "SELECT MAX(id_trx_nomor_ks) AS id_max FROM  trx_nomor_ks WHERE kode_unit_subunit = '{$unit}' AND tahun = '{$tahun}' AND jenis = 'SPP' " ;
        ...
        function get_nomor_spp_urut_by_nomor_spp($str_nomor_trx_spp){
            ...
            $query = "SELECT id_trx_nomor_ks FROM  trx_nomor_ks WHERE str_nomor_trx = '{$str_nomor_trx_spp}' " ;
            ...
            $q = $this->db->query($query);
                    ...
                    if(!is_null($q->row()->id_trx_nomor_ks)){
                       return $q->row()->id_trx_nomor_ks;
                    }else{
                        return '0';
                    }
        ...
        function check_dokumen_ks_by_str_trx($no_str_trx){
            ...
            $query = "SELECT posisi "
                    ...
                    . "AND t1.id_trx_ks = ( 
SELECT MAX(t2.id_trx_ks) FROM trx_ks AS t2 WHERE t2.id_trx_nomor_ks = t1.id_trx_nomor_ks )" ;
...
//            var_dump($q->num_rows());die;
        ...
        function get_data_untuk_pekerjaan($q,$kode_unit_subunit,$tahun){
            ...
            $query = "SELECT DISTINCT(untuk_bayar) FROM  trx_spp_ks_data WHERE untuk_bayar LIKE '%{$q}%' AND kode_unit_subunit = '{$kode_unit_subunit}' AND tahun = '{$tahun}' " ;
        ...
        function get_data_penerima($q,$kode_unit_subunit,$tahun){
            ...
            $query = "SELECT DISTINCT(CONCAT(penerima,'|',alamat,'|',nmbank,'|',nmrekening,'|',rekening,'|',npwp)) AS str_penerima FROM  trx_spp_ks_data WHERE penerima LIKE '%{$q}%' AND kode_unit_subunit = '{$kode_unit_subunit}' AND tahun = '{$tahun}' " ;
        ...
        function get_tgl_spm_kpa_by_spp($nomor_trx_spp){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    . "JOIN trx_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks "
                    . "WHERE trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spp}' "
                    . "AND trx_ks.posisi = 'SPM-DRAFT-KPA'" ;
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_tgl_spm_verifikator_by_spp($nomor_trx_spp){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    . "JOIN trx_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks "
                    . "WHERE trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spp}' "
                    . "AND trx_ks.posisi = 'SPM-FINAL-VERIFIKATOR'";
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_tgl_spm_kbuu_by_spp($nomor_trx_spp){
            ...
            $str = "SELECT trx_ks.tgl_proses FROM trx_nomor_ks "
                    . "JOIN trx_ks ON trx_nomor_ks.id_trx_nomor_ks = trx_ks.id_trx_nomor_ks "
                    . "WHERE trx_nomor_ks.str_nomor_trx = '{$nomor_trx_spp}' "
                    . "AND trx_ks.posisi = 'SPM-FINAL-KBUU'";
...
//            var_dump($str);die;
    ...
    //            var_dump($q->num_rows());die;
        ...
        function get_verifikator_by_spm($nomor_trx_spm){ 
            ...
            $str2 = "SELECT nmverifikator AS nm_lengkap,nipverifikator AS nomor_induk FROM trx_spm_ks_data "
...
//            var_dump($q->num_rows());die;
        ...
        function check_exist_ks($nomor_trx,$tahun){
            $this->db->where('str_nomor_trx',$nomor_trx);
            $this->db->where('tahun',$tahun);
            $query = $this->db->get('trx_nomor_ks');
            if ($query->num_rows()>0){
                return true;
            }else{
                return false;
        ...
        function get_kas_bendahara_by_spm($no_spm_ks){
            $q = "SELECT saldo FROM kas_bendahara WHERE no_spm = '{$no_spm_ks}' AND aktif = '1'" ;
            // echo $q ; die ;
            $r = $this->db->query($q) ;
