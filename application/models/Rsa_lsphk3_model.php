...
function search_rsa_lsphk3($kata_kunci=''){
		...
		if ($query->num_rows()>0){
         ...
         function check_dokumen_lsphk3($kode_unit_subunit,$tahun,$id){
			...
			//var_dump("SELECT posisi FROM trx_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND aktif = '1'  AND tahun = '{$tahun}'AND id_kuitansi='{$id}'");die;
            if($q->num_rows() > 0){
               return $q->row()->posisi ;
            }else{
                return '';
...
//                var_dump($result);die;
		...
		return $result ;
		 ...
		 function get_nomor_spp($kode_unit_subunit,$tahun,$id){
              $q = $this->db->query("SELECT str_nomor_trx AS nt FROM trx_nomor_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}'  AND id_kuitansi='{$id}' and aktif='1'");
			  //var_dump("SELECT str_nomor_trx AS nt FROM trx_nomor_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = 'SPP'  AND tahun = '{$tahun}'  AND id_kuitansi='{$id}' and aktif='1' ");die;
			 // var_dump($q->row()->nt);die;
            if($q->num_rows() > 0){
               return $q->row()->nt ;
            }else{
                return '' ;
    	...
    	//print_r("SELECT * FROM trx_spp_lsphk3_data WHERE str_nomor_trx = '{$str_nomor_trx}' AND id_kuitansi='{$id}'");die;
//            if($query->num_rows()>0){
                    return $query->row();
//            }else{
//                    return array();
//            }
		 ...
		 function lihat_ket($kode_unit_subunit,$tahun,$id){
           ///var_dump("SELECT ket FROM trx_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}'  AND tahun = '{$tahun}' AND id_kuitansi='{$id}' AND ket!=''");die;
		   $q = $this->db->query("SELECT ket FROM trx_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}'  AND tahun = '{$tahun}' AND id_kuitansi='{$id}' and ket !=''");
            //var_dump($q->num_rows());die;
            if($q->num_rows() > 0){
               return $q->row()->ket ;
            }else{
		 ...
		 function proses_nomor_spp_lsphk3($kode_unit,$data,$id){
...
//            echo "SELECT id_trx_nomor_lsphk3 FROM trx_nomor_up WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}' AND tahun = '{$tahun}' AND aktif='1' "; die;
            $q = $this->db->query("SELECT id_trx_nomor_lsphk3 FROM trx_nomor_lsphk3 WHERE kode_unit_subunit = '{$kode_unit_subunit}' AND jenis = '{$jenis}' AND tahun = '{$tahun}'  AND id_kuitansi='{$id}' AND aktif='1' ");
//            var_dump($q->num_rows());die;
            if($q->num_rows() > 0){
...
//            var_dump($str);die;
	...
	function get_lsphk3_unit_usul_verifikator($id_user_verifikator,$tahun,$kode_subunit){
            // print_r($kode_subunit); exit;
            foreach ($kode_subunit as $k => $v) {
                $str_kode_subunit[]= "a.kode_unit_subunit LIKE '".$v->kode_unit_subunit."%'";
            }
            $str_kode_subunit = " AND (".implode(" OR ", $str_kode_subunit).")";
     //        // $str_kode_subunit = " AND (rsa.trx_lsphk3.kode_unit_subunit LIKE '".implode("%' OR rsa.trx_lsphk3.kode_unit_subunit LIKE '", $kode_subunit)."%') ";
     ...
     //                . "ORDER BY rba.unit.kode_unit ASC";
				$query = "SELECT LEFT(a.kode_unit_subunit,2) AS kode_unit, COUNT(a.posisi) AS jumlah, a.posisi, a.tahun FROM trx_lsphk3 a LEFT JOIN rsa_kuitansi_lsphk3 b ON a.id_kuitansi = b.id_kuitansi WHERE (a.aktif = '1' OR a.aktif IS NULL) AND (a.tahun LIKE '{$tahun}' OR a.tahun IS NULL) AND b.jenis LIKE 'L3'".$str_kode_subunit." GROUP BY a.kode_unit_subunit";
...
//               var_dump($result);die;
		...
		function get_lsphk3_unit_usul_verifikator_nk($id_user_verifikator,$tahun,$kode_subunit){
             foreach ($kode_subunit as $k => $v) {
                $str_kode_subunit[]= "a.kode_unit_subunit LIKE '".$v->kode_unit_subunit."%'";
            }
            $str_kode_subunit = " AND (".implode(" OR ", $str_kode_subunit).")";
...
//               var_dump($result);die;
	 ...
	 function get_lsphk3_subunit_usul_verifikator($id_user_verifikator, $tahun, $kode_subunit){
           	foreach ($kode_subunit as $k => $v) {
                $str_kode_subunit[]= "a.kode_unit_subunit LIKE '".$v->kode_unit_subunit."%'";
            }
            $str_kode_subunit = " AND (".implode(" OR ", $str_kode_subunit).")"; 
     ...
     //                . "ORDER BY rba.subunit.kode_subunit ASC";
	 	...
	 	$query = "SELECT kode_unit_subunit AS kode_subunit, COUNT(a.posisi) AS jumlah, a.posisi, a.tahun FROM trx_lsphk3 a LEFT JOIN rsa_kuitansi_lsphk3 b ON a.id_kuitansi = b.id_kuitansi WHERE (a.aktif = '1' OR a.aktif IS NULL) AND (a.tahun LIKE '{$tahun}' OR a.tahun IS NULL) AND b.jenis LIKE 'L3'".$str_kode_subunit." GROUP BY a.kode_unit_subunit";
...
//                var_dump($result);die;
		...
		//NK
		function get_lsphk3_subunit_usul_verifikator_nk($id_user_verifikator,$tahun,$kode_subunit){
...
//            }else{
        ...
        */
		...
		//get keluaran
		function get_keluaran($nomor_trx,$id){
            $this->db->where("str_nomor_trx_spp",$nomor_trx);
			$this->db->where("id_kuitansi",$id);
            $query = $this->db->get('trx_lsphk3_keluaran');
            return $query->result();
        }
		...
		function insert_keluaran($data){
            return $this->db->insert_batch("trx_lsphk3_keluaran",$data);
        }
 
	 function get_kuitansi($id){
		   $q = $this->db->query("SELECT sumber_dana FROM rsa_kuitansi_lsphk3 WHERE id_kuitansi='{$id}'");
            if($q->num_rows() > 0){
               return $q->row()->sumber_dana ;
            }else{
                return '';
            }
        }
