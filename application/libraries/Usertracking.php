...
class Usertracking
{
 // private $session_id;
  ...
  public function track_this()
  {
    ...
    $this->CI->load->library('check_session');
    //get the data
    $input_data = array();
	if($input_data['session_id']=="NULL" || $input_data['session_id']=="NULL" ){
		 $input_data['session_id'] = 500;
		 $input_data['user_identifier'] = 'GUEST';
	}else{
		$input_data['session_id'] = $_SESSION['rsa_level'];
		$input_data['user_identifier'] = $this->CI->session->userdata('rsa_username');
	}
   ...
   // $input_data['timestamp'] = time();
	...
	if($input_data['user_identifier']==NULL){
		$input_data['user_identifier'] = GUEST;
	}
