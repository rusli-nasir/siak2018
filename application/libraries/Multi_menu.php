	...
	{	    
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
				...
				if ($item['notif']  !== null){
					if ($this->jumlah_notifikasi->$item['notif'] == 0){
						$label .= '<span class="label label-primary pull-right">'.$this->jumlah_notifikasi->$item['notif'].'</span>';
					}else{
						$label .= '<span class="label label-warning pull-right">'.$this->jumlah_notifikasi->$item['notif'].'</span>';
					}
				}
